...
Current catalog: **271 skills** across 21 categories, 17 commands (plus 3 permanent aliases), 31 hooks, 23 agents. The 40 v3.x deprecation shims were removed in v3.2.0.
...
`configs/platform-defaults.json` is the **single place a per-platform install-time behavioral default is edited** (reasoning effort, a default-model pin, an approval or autonomy policy). Everything that consumes such a default is DERIVED from it. Full schema, rules, and worked procedure: [`configs/README.md`](configs/README.md).
...
## Time-Bounded Agent Autonomy

`nexus-hub autonomy` is the public control surface for temporarily relaxing a supported agent platform's project-scoped execution policy. The CLI defaults to `status`; `enable` defaults to the public `edits` tier; `disable` restores the immediate pre-enable configuration; and `revert` restores the latest durable backup. Every platform capability is declared by an `IntegrationBase.config["autonomy"]` descriptor, and descriptorless platforms are reported as unavailable rather than inferred from a similar product.

The two public tiers are intentionally narrow. `edits` permits file edits while execution-trigger tools still require approval. `full` permits the descriptor's broader project-scoped execution policy, requires an interactive terminal and a typed project-directory confirmation, and never disarms Nexus-Hub hooks. Both tiers require a clean worktree on a non-protected branch, render the exact configuration diff before confirmation, expire after a bounded TTL, create a backup, and append an audit record. Global-scope autonomy descriptors are rejected by the core.

Autonomy is independent from the read-only permission baseline documented in [`docs/permissions-research.md`](docs/permissions-research.md). A platform may have a verified autonomy descriptor without receiving a broad global allowlist, and the absence of a descriptor is never patched around in the CLI or the VS Code monitors. The monitors are status and control surfaces only: every mutation shells through `nexus-hub autonomy`, so the Python core remains the single policy engine.

The existing `[[agent-access-policy]]`, `[[ai-billing-safeguards]]`, and `[[agentic-endpoint-hardening]]` skills cover least privilege, spending limits, and endpoint hardening respectively. Phase 5 does not add an `agent-autonomy-policy` skill because the current work defines the WHAT and HOW of one bounded mechanism, not enough recurring evidence for separate WHEN guidance; usage can justify that follow-on later.
...
| `scripts/nexus_hub_cli.py` and `scripts/lib/autonomy.py` | No new edit for autonomy subcommands - both installers already copy the CLI entry point explicitly and recursively copy `scripts/lib/`. Add an installer step only if the implementation moves to a new top-level script. | All installations with the `nexus-hub` CLI; platform availability remains descriptor-gated. |
