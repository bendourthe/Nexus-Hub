...
def test_bash_precheck_only_succeeds() -> None:
    ...
    proc = subprocess.run([BASH, "install.sh"], cwd=str(REPO_ROOT), env=env, **_RUN_KW)
...
def test_bash_standalone_extracts_and_hands_off(tmp_path: Path) -> None:
    ...
    proc = subprocess.run(
        [BASH, "install.sh", "alpha", "beta"], cwd=str(REPO_ROOT), env=env, **_RUN_KW,
    )
...
def test_bash_in_repo_delegates(tmp_path: Path) -> None:
    ...
    proc = subprocess.run([BASH, "install.sh", "foo"], cwd=str(tmp_path), **_RUN_KW)
    assert proc.returncode == 0, (proc.stdout, proc.stderr)
...
def test_ps_precheck_only_succeeds() -> None:
    ...
    proc = subprocess.run(
        ...
        cwd=str(REPO_ROOT), env=env, **_RUN_KW,
    )
    assert proc.returncode == 0, proc.stderr
    assert "precheck" in (proc.stdout + proc.stderr).lower()
