...
def test_the_rubric_has_the_coverage_depth_criterion():
    assert "## The eleven criteria" in RUBRIC_TEXT
...
def test_no_stale_criteria_count_survives():
    # The rubric grew to eleven (v3.16.7 painted-surface integrity); a surface
    # still claiming an older count would tell the grader to skip new criteria.
    for name, text in (("SKILL.md", SKILL_TEXT), ("visual-qa-rubric.md", RUBRIC_TEXT)):
        ...
        assert "ten criteria" not in text.lower(), f"{name} still says ten criteria"
    assert "all eleven criteria" in SKILL_TEXT
