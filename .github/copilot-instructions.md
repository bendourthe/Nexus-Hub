...
1. **New file under `scripts/<name>.{py,js}`**: MUST be registered in BOTH `scripts/installer.sh` (around line 1395, next to `generate_report.py`) AND `scripts/installer.ps1` (around line 1656). The installer copies scripts by explicit name, never by folder. Copy the pattern used for `generate_report.py` and `compile_deep_research.py`.
