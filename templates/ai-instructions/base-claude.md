...
- **MANDATORY**: When using the PowerShell tool, the same `description` requirement applies. Provide it as plain text only - no borders, no `#` characters, no manual formatting. The `format-powershell-description.py` hook prepends the description as a comment block at the top of the script so it stays visible in the truncated approval-dialog preview, and auto-approves single-line read-only pipelines that match `PowerShell(...)` allow patterns. The `require-powershell-description.sh` hook blocks commands that omit a description.
