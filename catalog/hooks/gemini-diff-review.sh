...
if ! command -v gemini >/dev/null 2>&1; then
    ...
    echo "[gemini-diff-review] Install Gemini CLI (https://github.com/google/generative-ai-cli) or set NEXUS_DIFF_REVIEW_DISABLE=1 to silence this warning." >&2
    exit 0
fi
...
if [ "$DIFF_BYTES" -gt "$MAX_BYTES" ]; then
    ...
    echo "[gemini-diff-review] Raise the cap with NEXUS_DIFF_REVIEW_MAX_BYTES=N or commit fewer files at a time." >&2
    exit 0
fi
...
case "$VERDICT" in
    ...
    BLOCK)
        ...
        echo "$RESPONSE" | tail -n +2 >&2
        echo "" >&2
        echo "[gemini-diff-review] Commit refused. Fix the issue, or bypass this commit with: NEXUS_DIFF_REVIEW_DISABLE=1 git commit ..." >&2
        exit 1
        ;;
    *)
