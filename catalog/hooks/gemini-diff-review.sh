...
#
# Install:
#   /install-pre-commit-review-hook --platform=gemini   (run from inside the target repo)
#
# Per-commit bypass:
...
if ! command -v gemini >/dev/null 2>&1; then
    echo "[gemini-diff-review] WARNING: gemini CLI not found on PATH; skipping review." >&2
    echo "[gemini-diff-review] Install Gemini CLI (https://github.com/google/generative-ai-cli) or set DEVAI_DIFF_REVIEW_DISABLE=1 to silence this warning." >&2
    exit 0
fi
...
if [ "$DIFF_BYTES" -gt "$MAX_BYTES" ]; then
    echo "[gemini-diff-review] WARNING: staged diff is ${DIFF_BYTES} bytes (cap=${MAX_BYTES}); skipping review." >&2
    echo "[gemini-diff-review] Raise the cap with DEVAI_DIFF_REVIEW_MAX_BYTES=N or commit fewer files at a time." >&2
    exit 0
fi
...
RESPONSE=$(printf '%s' "$DIFF" | gemini -p "$PROMPT" 2>/dev/null || true)
if [ -z "$RESPONSE" ]; then
    echo "[gemini-diff-review] WARNING: gemini CLI returned no output; allowing commit." >&2
    exit 0
fi
...
case "$VERDICT" in
    PASS)
        ...
        ;;
    WARN)
        echo "[gemini-diff-review] WARN:" >&2
        echo "$RESPONSE" | tail -n +2 >&2
        echo "" >&2
        echo "[gemini-diff-review] Commit allowed; review the warnings above and consider amending." >&2
        exit 0
        ;;
    BLOCK)
        echo "[gemini-diff-review] BLOCK:" >&2
        echo "$RESPONSE" | tail -n +2 >&2
        echo "" >&2
        echo "[gemini-diff-review] Commit refused. Fix the issue, or bypass this commit with: DEVAI_DIFF_REVIEW_DISABLE=1 git commit ..." >&2
        exit 1
        ;;
    *)
        echo "[gemini-diff-review] WARNING: unparseable verdict from gemini (expected PASS|WARN|BLOCK); allowing commit." >&2
        exit 0
        ;;
esac
