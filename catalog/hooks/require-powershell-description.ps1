<#
.SYNOPSIS
    PowerShell parity for require-powershell-description.sh.

.DESCRIPTION
    PreToolUse hook that BLOCKS (exit 2) a PowerShell tool call carrying no
    description.
    ...
    The format-powershell-description.py hook runs before this one and only adds
    the prefix to commands that are NOT auto-approved; auto-approved commands carry
...
# this is not a PowerShell tool call we can assess (an empty payload, a valid
# payload for a DIFFERENT tool such as Write, or a malformed one), so allow rather
# than block. Catching only the parse error is not enough: a well-formed payload
# carrying no command would otherwise fall through to the block below and refuse an
# unrelated tool call.
