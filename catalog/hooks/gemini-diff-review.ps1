# gemini-diff-review.ps1 - opt-in git pre-commit hook (Nexus-Hub).
# Pipes the staged diff through `gemini -p` (Google Gemini CLI) for an LLM
# review of hardcoded secrets, debug artifacts, unfinished TODOs, and large
# commented-out code blocks.
#
# Independent of the Claude / Codex / OpenCode variants - calls the Gemini CLI only.
#
# PowerShell parity for gemini-diff-review.sh: same bypass paths, same diff cap,
...
# --- Locate gemini CLI ---------------------------------------------------
$cliCmd = Get-Command gemini -ErrorAction SilentlyContinue
if (-not $cliCmd) {
    Write-Host "[gemini-diff-review] WARNING: gemini CLI not found on PATH; skipping review." -ForegroundColor Yellow
    Write-Host "[gemini-diff-review] Install Gemini CLI (https://github.com/google/generative-ai-cli) or set NEXUS_DIFF_REVIEW_DISABLE=1 to silence this warning." -ForegroundColor Yellow
    exit 0
}
...
if ($diffBytes -gt $maxBytes) {
    Write-Host "[gemini-diff-review] WARNING: staged diff is $diffBytes bytes (cap=$maxBytes); skipping review." -ForegroundColor Yellow
    Write-Host "[gemini-diff-review] Raise the cap with `$env:NEXUS_DIFF_REVIEW_MAX_BYTES=N or commit fewer files at a time." -ForegroundColor Yellow
    exit 0
}
...
# --- Run gemini (fail-open on any error) ------------------------------------
$response = $null
try {
    $response = $diff | gemini -p $prompt 2>$null
} catch {
    $response = $null
}
if ([string]::IsNullOrWhiteSpace($response)) {
    Write-Host "[gemini-diff-review] WARNING: gemini CLI returned no output; allowing commit." -ForegroundColor Yellow
    exit 0
}
...
switch ($verdict) {
    ...
    "WARN" {
        Write-Host "[gemini-diff-review] WARN:" -ForegroundColor Yellow
        $response -split "`n" | Select-Object -Skip 1 | ForEach-Object { Write-Host $_ }
        Write-Host ""
        Write-Host "[gemini-diff-review] Commit allowed; review the warnings above and consider amending." -ForegroundColor Yellow
        exit 0
    }
    "BLOCK" {
        Write-Host "[gemini-diff-review] BLOCK:" -ForegroundColor Red
        $response -split "`n" | Select-Object -Skip 1 | ForEach-Object { Write-Host $_ }
        Write-Host ""
        Write-Host "[gemini-diff-review] Commit refused. Fix the issue, or bypass this commit with: `$env:NEXUS_DIFF_REVIEW_DISABLE=1; git commit ..." -ForegroundColor Red
        exit 1
    }
    default {
        Write-Host "[gemini-diff-review] WARNING: unparseable verdict from gemini (expected PASS|WARN|BLOCK); allowing commit." -ForegroundColor Yellow
        exit 0
    }
}
