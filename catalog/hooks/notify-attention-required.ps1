<#
.SYNOPSIS
    PowerShell parity for notify-attention-required.sh.

.DESCRIPTION
    Trigger A of the end-of-task notification contract: the agent is BLOCKED on
    ...
    prompt while the human is in another window. The old behavior notified on
    every turn ending, which buried this case in noise instead of surfacing it.
...
.NOTES
    Runtime controls (all checked on EVERY invocation):
      Disable by name:          $env:NEXUS_DISABLED_HOOKS = "notify-attention-required"
...
$hookName = "notify-attention-required"
...
Send-NexusNotification -Title "Claude Code" -Message ("Needs your input - " + (Get-NexusNotifyLabel))
