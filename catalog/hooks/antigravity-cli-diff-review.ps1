...
# transition (Gemini CLI stops serving non-enterprise users on 2026-06-18).
#
# Install (PowerShell):
#   /install-pre-commit-review-hook --platform=antigravity
#
# Per-commit bypass:
...
if ($diffBytes -gt $maxBytes) {
    Write-Host "[antigravity-diff-review] WARNING: staged diff is $diffBytes bytes (cap=$maxBytes); skipping review." -ForegroundColor Yellow
    Write-Host "[antigravity-diff-review] Raise the cap with `$env:NEXUS_DIFF_REVIEW_MAX_BYTES=N or commit fewer files at a time." -ForegroundColor Yellow
    exit 0
}
...
switch ($verdict) {
    ...
    "WARN" {
        Write-Host "[antigravity-diff-review] WARN:" -ForegroundColor Yellow
        $response -split "`n" | Select-Object -Skip 1 | ForEach-Object { Write-Host $_ }
        Write-Host ""
        Write-Host "[antigravity-diff-review] Commit allowed; review the warnings above and consider amending." -ForegroundColor Yellow
        exit 0
    }
    "BLOCK" {
        Write-Host "[antigravity-diff-review] BLOCK:" -ForegroundColor Red
        $response -split "`n" | Select-Object -Skip 1 | ForEach-Object { Write-Host $_ }
        Write-Host ""
        Write-Host "[antigravity-diff-review] Commit refused. Fix the issue, or bypass this commit with: `$env:NEXUS_DIFF_REVIEW_DISABLE=1; git commit ..." -ForegroundColor Red
        exit 1
    }
    default {
