...
if (($command -cmatch '(^|[;&|]|\s)git(\s|$).*config') -and
    ($command -cmatch 'core\.(hooksPath|fsmonitor)') -and
    -not ($command -cmatch '--(get|get-all|get-regexp|get-urlmatch|list|unset|unset-all)(\s|=|$)')) {
...
if ($env:NEXUS_PROTECTED_BRANCHES -and
    ...
    ($command -cmatch '(^|[;&|]|\s)git\s+commit(\s|$)')) {
