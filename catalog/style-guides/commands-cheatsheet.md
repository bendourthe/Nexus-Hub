...
- **Permanent alias** -- a convenience command that is not deprecated but forwards to a scoped command for ergonomics (currently `constitution` -> `/spec constitution`, `commit` -> `/update commit`). Treat as active; mark as an alias.
