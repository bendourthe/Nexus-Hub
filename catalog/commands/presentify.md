...
- Out of scope: video / audio embedding. Scanned / image-only PDF reading (local OCR + agent-vision fallback), PDF image and vector-figure extraction, and native PowerPoint / Word chart objects are all supported by the extractor. See the skill's `references/extraction-runbook.md`.
