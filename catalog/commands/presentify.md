...
```
/presentify <file-or-folder...> [using the style <description> | --style <description>] [--theme <name-or-path>] [--layout <full|standard|portrait|description>] [--images <procedural|stock|ai|auto|none>] [--interactivity <restrained|balanced|rich>] [--out <path>] [--mode <auto|deck|report|compile>]
```
...
- **`--images`** (optional): the imagery tier - one of `procedural` (Tier 1, the default: original inline SVG / CSS visuals, zero-outbound), `stock` (Tier 2, opt-in: license-free, free-for-commercial-use stock images / video fetched and base64-embedded at build time), `ai` (Tier 3, opt-in: images generated by a LOCAL commercially-clean model), `auto` (procedural, plus stock only when consent is given), or `none` (typography / layout / color only, no added visuals). The natural form `using ... images` is equivalent (e.g. `using license-free stock images`). A named tier binds and skips the imagery menu below. The stock and AI tiers are opt-in and, for any outbound path, require explicit consent (wired in a later phase); the default `procedural` and every non-interactive run stay fully offline.
...
## Choosing imagery and interactivity (when no imagery/interactivity is given)

After the output aspect is resolved, `/presentify` resolves imagery and interactivity - the visual voice of the output - the same way it resolves style and layout. It asks two short questions.

Imagery tier (which visuals the output carries):
...
2. **License-free stock** - opt-in: fetch highly-relevant, free-for-commercial-use images / video from openly-licensed sources at build time, verify each license, base64-embed them (so the output stays offline), and credit them.
3. **Local AI-generated** - opt-in: generate original images with a LOCAL, commercially-clean model runtime; no hosted generation service is ever called.
4. **A mix** - procedural as the base, with stock or local-AI accents where they add value.

Interactivity level (how the output responds to the reader):
...
The stock and AI tiers are OPT-IN and, for any outbound path, require explicit consent for the build-time network use (the consent prompt is wired in a later phase); a non-interactive or headless run never fetches or generates and stays on Tier 1. If the menu cannot be answered, the agent picks imagery = procedural (Tier 1, offline) and a content-aware interactivity level (a deck or data story leans balanced; a report leans restrained), and records both. A named `--images` / `using ... images` and `--interactivity` skip the respective menu entirely. Both the resolved imagery tier and interactivity level are recorded in the output's design-record HTML comment.
