...
- **Per-version docs structure**: check the active version's tree against the `docs/v<MAJOR>/v<MAJOR>.<MINOR>/` scheme (with `plans/` and `comparisons/` subdirs) and refactor any deviation - relocate stray comparison reports into `comparisons/`, and normalize the archive to `docs/archive/v<MAJOR>/v<MAJOR>.<MINOR>/`.
