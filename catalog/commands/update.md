...
2a2. **Closed-minor archival report**: list every minor under `docs/releases/v<MAJOR>/v<MAJOR>.<MINOR>/` that is fully closed - its `known-gaps.md` proves no open work (no in-progress or open status, no unchecked box, no `NI-`/`DF-`/`BG-`/`WN-`/`MT-`/`QG-` id under Open Items) and none of its plans carries an unchecked task line - yet still holds `plans/` or `comparisons/` in the active tree. Those two subtrees move to `docs/archives/`; `known-gaps.md` stays active so the next `/plan` reads it without a directory hop. For Nexus-Hub, `python scripts/check_docs_retention.py` produces this list; it is repo-internal tooling and is not installed, so in any other repository apply the same closure test by reading the files. A minor with no `known-gaps.md` at all is reported as inferred, never as proven closed. Self-gates to a silent no-op in a repository with no `docs/releases/` tree. The move itself belongs to `[[docs-layout-refactor]]`.

    **Report here; move under `[[docs-layout-refactor]]` with confirmation.** Relocating a directory tree and repairing its references is not something a release flow performs unattended, so this step surfaces the list and stops. The detector is deliberately conservative: an unparseable or ambiguous register reads as OPEN, and a minor with no register at all is reported separately as inferred rather than proven, because a false "closed" archives live work while a false "open" costs one advisory line. Those two errors are not symmetric, which is why the check leans to the cheap one.
...
5a. **Queued-plan impact review** via `[[plan-queue-assessment]]`: evaluate the state this release establishes against every still-open queued plan, and report which of them the release changed, naming the affected task lines. A release is the single largest step-change to the codebase that queued plans were written against, so it is the moment their assumptions are most likely to have gone stale. This step **reports; it does not edit a queued plan** - the finding is recorded so that plan's own next `/implement` re-assessment inherits it. A queue with nothing affected produces an explicit statement to that effect, never an omitted section, because an omitted section is indistinguishable from a skipped check. It self-gates to a silent no-op in a repository with no `docs/releases/*/*/plans/` tree. Enumerate with `python scripts/enumerate_plan_queue.py`; do not restate the classification or ranking rules, which that skill owns.

    The review MAY also surface a re-ordering recommendation. Acting on one is a **separate, explicitly confirmed** operation and never part of the release flow: see the renumber procedure in `[[plan-queue-assessment]]`. A release must not silently renumber a queued plan, because that rewrites version identity that merged pull requests and published documentation already reference.
...
6.5. **Plan worktree integration and teardown**: when the release closes a plan that ran in its own worktree (the default since v4.12.1, see `[[implement-phase]]`), confirm the worktree is fully integrated and then removed, before the release commit. Three checks in order, each fail-closed. First, the plan branch is **fully merged** into the integration branch: it appears in `git branch --merged <base>` and `git log <base>..<branch>` is empty. Second, the worktree has **no uncommitted or untracked work**: `git -C <worktree> status --porcelain` is empty, and a non-empty result stops the release rather than being force-removed. Third, once both hold, the worktree is removed via `[[using-git-worktrees]]`, its directory and branch are deleted, `git worktree prune` runs, and `git worktree list` no longer shows the path. Quote the result. This step exists because the cost of parallel plans is otherwise permanent: ten shipped plans leave ten stale checkouts, each pinning objects, each editable by mistake, and each indistinguishable from the real repository at a glance. Self-gates to a silent no-op when the release closed no worktree-backed plan. Never remove a worktree before its merge is green and merged, and never with `--force` past a dirty tree; both convert unfinished work into deleted work.
