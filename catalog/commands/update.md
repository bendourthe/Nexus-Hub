...
- Treat `documentation` as an alias for `docs`, so `/update documentation` runs the same handbook refresh.
...
- **Interactive handbooks against the actual candidate**: delegate to `technical-documentation` and its `references/handbook-refresh.md`. Recursively inventory `docs/handbooks/`, preserve the project's Markdown/HTML or topic-folder layout, refresh content against the requested working tree or release candidate, and regenerate every affected eligible HTML with its detailed reading page plus title/top-menu Presentation Mode. Global entry starts slide 1. Reuse approved branding/settings; apply `document-to-interactive-html`'s shared dual-view contract. Missing/stale output or unverified required views keep the refresh incomplete; unchanged-but-verified and explicit opt-outs require recorded evidence. Do not assume `main`, or a literal `markdown/` subfolder, is the only source.

When the scope is `release`, run this reconciliation as the FIRST content step after the existing integration prerequisites, before the version bump. Recheck any output affected by the later version changes before release completion. A release whose only documentation change is the version/count bump has not run `docs`.
...
2a. **Generated-doc regenerate-and-fail-on-stale**: run `technical-documentation`'s handbook refresh procedure and every other repository-owned documentation generator, then their read-only check commands. Include all eligible `docs/handbooks/` HTML regardless of folder layout, verify both views under `document-to-interactive-html`'s shared contract, and fail completion for missing, stale, or unverified required output. A missing generator for existing handbook HTML requires migration, not a no-op. Record a no-op only after discovery proves no eligible generated documentation exists. Recheck after version metadata changes.
