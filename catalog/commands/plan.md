...
- **Refresh the ids; inherit the placements.** The refresh answers which model ids exist, not which tier this project ranks them in - that is a recorded judgment, and the bundled `last-known-model-map.json` is its input on the fresh path, not merely the offline fallback. Run `model-map.{sh,ps1} diff <candidate.json>` alongside `validate`. Exit 3 lists every id that changed tier and stops the step: restore the snapshot placement, or keep the move and state its reason in the plan's map section. A reshuffled column passes `validate`, so skipping `diff` is how a tier column silently inverts between plans.
