...
For `full`, produce the twelve-section report below and write it to `docs/<version>/analysis.md` (resolve `<version>` per the `[[docs-layout-refactor]]` Version-directory resolution - canonically `docs/v<MAJOR>/v<MAJOR>.<MINOR>/analysis.md`). For a focused scope, emit only the mapped section(s) inline and offer to append them to the report file.
