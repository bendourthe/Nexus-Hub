...
3. **Detect** the required living tree: `docs/handbooks/` with its existing source/output map (legacy `markdown/` + `html/` or topic layout) and `docs/decisions/`. Also detect living `docs/README.md` and `docs/todos.md` (`docs/DEVLOG.md` is Step 5).
4. **Act if missing**: scaffold only the missing pieces. Create `docs/handbooks/README.md` and `docs/decisions/` (the ADR tree; do not invent records). Delegate a missing handbook tree to `[[technical-documentation]]` through `references/handbook-refresh.md` to author a focused overview and relevant technical topics from actual repository evidence. Reuse existing layouts and native generators; do not fabricate architecture or call empty source folders complete. Create `docs/README.md` and `docs/todos.md` with real content if absent. Never overwrite inherited files. Never invent `docs/testing/` or `docs/validation/`.
...
- [ ] `docs/handbooks/` with its existing source/output map (legacy `markdown/` + `html/` or topic layout) and `docs/decisions/` exist; `docs/testing/` and `docs/validation/` were not invented.
