...
12. **Slide-mode integrity** (v3.18.3, `references/slide-navigation.md`). Apply whenever presentation is included, including an HTML that initially opens as a scrolling page. The legacy scorer only recognizes its older `nav=slides` markup; browser verification must independently cover the embedded presentation. Four metric groups:
