...
The eleven criteria above are two of four layers. Naming all four matters because without the vocabulary, "QA passed" silently means whichever layers happened to run - and a real session (2026-08-13) shipped an audience mismatch, an oversized estimate, a redundant argument, and decorative visuals from a run whose structural and behavioral checks were all green. Those layers ask whether the page WORKS. Nothing was asking whether it served its reader's decision.
