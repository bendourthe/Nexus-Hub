...
For both-view source coverage, positive reference comparison, adjacent-section themes, continuous fit, and final-hash evidence, apply [dual-view-handbooks.md](dual-view-handbooks.md). A structural score is a partial check, not a visual or production-ready verdict.
...
Each criterion lists what it checks, the observable metric, the check kind, and the severity of a failure. Criteria 1-5 came from v3.15.4 Phases 1-4; criteria 6-7 grade the `references/responsive-typography.md` contract and criterion 8 the `references/svg-diagram-quality.md` contract, both added in v3.16.5; criterion 9 was added by the v3.16.5 errata after a real render session found three defect classes no contract covered; criterion 10 (v3.16.6) grades the coverage-depth choice the round-2 intake records; criterion 11 (v3.16.7) was added after a class-collision regression blanked a JS-painted stage with zero console errors and a clean structural pass; criterion 12 (v3.18.3) grades presentation whenever included, including dual-view pages.
...
12. **Slide-mode integrity** (v3.18.3, `references/slide-navigation.md`). Apply whenever presentation is included, including an HTML that initially opens as a scrolling page. The scorer checks the retained handbook record against actual dual-view markup, with a mandatory absence guard for presentation=no. It retains the older `nav=slides` checks for legacy documents. Browser verification independently covers every enabled slide/state; a structural pass does not prove readable fit or behavior. Four metric groups:

    (a) **Slide fit.** No slide's content overflows its stage at ANY of the four QA viewports, and the **1366x768 leg is checked explicitly** because that is where the root clamp pins and text is largest relative to the stage - fit there alone does not establish fit at intermediate sizes or other aspect ratios; use the matrix and compact-mode boundary in `dual-view-handbooks.md`. No clipped text, and no inner scrollbar except an explicitly-declared scrollable region carrying a visible affordance and a design-record note. Metric: per slide, measure the active `[data-dv-slide]` and all painted descendants against `.dv-stage` at each supported desktop viewport, plus the declared-exception list; legacy `.slide-inner` keeps its own equivalent check. Inspect both sides of compact breakpoints and retain readable reflow there. Kind: RENDER PROBE for the per-slide measurement; STRUCTURAL (`slide-fit`) for the stage-sizing and scroll-lock declarations. Severity: HIGH (undeclared overflow hides content the reader cannot reach).
...
## Independent reading and presentation coverage

Page verbosity remains criterion 10. Presentation depth is a separate comparison with the page-to-slide/state map: concise keeps the essential message and caveats with explicit omissions, balanced covers every major topic, and deep dive covers every substantive page unit. Preserve the logical source-slide ceiling, saved themes and reading order. Inspect every source and output instance; zero selected sections/slides/figures is a failure, not an empty passing sample. A page-only opt-out runs factual, visual and functional page QA plus deck/CTA absence; enabled-only checks are N/A. Use `dual-view-handbooks.md` for shared coverage and fit ownership.
