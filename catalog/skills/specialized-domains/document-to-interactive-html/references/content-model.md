...
## Retained reading and presentation assembly

A model with `presentation` uses the retained assembler behind `scripts/build_presentation.py`. Models without it retain the legacy plain-draft path. Narrative extraction and AI composition happen before assembly; build and `--check` never call a model, execute a source command, or fetch an asset.

- Every section and block has a unique stable `id` within its owner. `sources` contains root-relative `path` objects; `retained_inputs` lists additional root-relative evidence/assets whose bytes affect freshness. Preserve the originals and extraction worksheet beside the model.
- `presentation.enabled` is an explicit boolean. `false` produces the complete reading page without slide runtime, slide CSS, or entry controls. `true` requires `slides`, positive `slide_budget`, `depth` (`concise`, `balanced`, `deep-dive`), `theme` (`light`, `dark`, `mixed`), and the saved `theme_sequence`. An existing source deck also records `source_slide_count`; neither ceiling can be exceeded. Resolve mixed themes during authoring, never inside build/check.
- Each slide has a stable `id`, `source_ids` in intended order, optional semantic `heading`, and named `composition`. Optional `unit_ids` maps each source section to its selected block IDs. Absent selection includes every block. The reading page always includes every unit. Balanced includes every major section; deep dive includes every unit; concise may omit sections. Every omitted section or `section/unit` has an explanation in `presentation.omissions`. This mapping does not certify whether the editorial selection is appropriate; authoring QA compares it with the independent inventory.
- `design` retains the visual brief, intake provenance, brand/style decisions, and a `reading_themes` sequence resolved from visible section order. `aliases` maps merged anchors to retained section IDs without consuming theme positions. Reserved runtime anchor names are rejected. Slide themes are independent of reading alternation.
- Optional `section.fragment` points to passive authored HTML with text-node slots such as `{{unit:acquisition-text}}`. Each required unit appears exactly once. IDs and local references are namespaced for each rendered instance. A slide may supply `fragments` keyed by its source section for a separate arrangement of selected units. This preserves bespoke compositions rather than freezing one generic layout.
- `design.stylesheet` points to retained CSS; `design.layout` saves full/standard/portrait reading canvas independently of deck dimensions. `design.brand_asset` points to a passive SVG lockup; optional `brand_variants` maps light/dark to supplied asset variants with unchanged mark/wordmark geometry. **The key is the SURFACE the lockup is drawn on, not the artwork's own colour**: `brand_variants.dark` is the asset placed on a DARK slide or section, so it is normally the light-coloured artwork, and `brand_variants.light` is normally the dark-coloured artwork. Reading the key as the artwork's colour inverts every surface at once and renders the wordmark invisible on both themes; the rendered brand check fails that build rather than shipping it. A slide can set `brand: false` when measured space or an existing cover lockup makes the extra logo inappropriate. Embedded font data is supported. External CSS resources, active HTML, arbitrary script attributes, SVG executable content, path traversal, drive/stream paths, hard links, and linked ancestors are rejected before output replacement. Complex SVGs belong in retained figure assets rather than unvalidated fragment markup.
- `figures` maps IDs to immutable definitions referenced by `figure` blocks. Simple bar/line definitions use named aligned series and finite numbers or `null`. Nonlinear axes and unsupported chart families require a faithful authored SVG or an enhanced original, never conversion to a misleading bar chart. `svg` and `map` definitions use `asset`; maps also have the complete labeled `regions` list. Raster images use validated embedded `data_uri` values. See `figure-reconstruction.md` for fidelity and geometry evidence.

### Build and check contract

Run the existing builder with the retained model and output paths; add `--root` when their common authoring root differs from the model directory. `--check` performs the same deterministic rendering and compares existing bytes and the adjacent `.html.build.json` record without modifying files. Missing output, changed declared source, changed builder/runtime/theme, malformed model, or stale record yields a nonzero exit. Repeated builds are byte-identical; output contains no wall-clock timestamps or QA scores.

Output and record destinations must be distinct from all retained inputs and stay under the root. An existing output can be rebuilt only when its hash matches its owned record, or the caller deliberately supplies its current `--expected-output-sha256`. A malformed/unowned record is never replaced. The assembler takes an exclusive local writer lock, rechecks sources, dependencies and destinations, then atomically replaces each file. The HTML and its sidecar are two replacements, not a filesystem-wide transaction: interruption between them leaves a detectable stale pair, requiring a rebuild before delivery. Hash checks and a cooperative writer lock detect observed concurrent edits; they do not claim isolation against an adversary who can rename directories between filesystem operations. Use an isolated authoring workspace for untrusted sources.

Three further fields exist so an annotation can be tied to what it names rather than merely placed near it. On a NOTE, `arrows` is a list of `[u, v]` targets in the panel's own data space: each draws a thin leader from the label's box to that point, stopping short of it, in the note's own ink. A label naming specific markers without one is a sentence near some dots rather than a sentence about them, and the reader has to guess the mapping. When several leaders leave one label they are ordered by target position and spread along the facing edge of the label's box, because a fan clamped to a single origin folds over itself. Also on a note, `anchor: "start"` makes `u` name the label's LEFT edge instead of its centre, which is what a label introducing a reference line from the left of the plot requires. On a PANEL, `ylabelStack: true` sets the axis name and its unit as one rotated two-line group in the gutter instead of floating the unit above the plot; use it whenever the top tick would otherwise sit under the unit, and use it consistently across panels that must share a left boundary, since the gutter width follows from it.

Vertical axis bands are derived from the rendered type size, never fixed: a band sized for an 11.5px tick label cannot hold a 21px one, and the axis title then lands on the numbers. Derive them from MEASURED box metrics rather than the nominal size. A y tick label baselined on the axis hangs roughly a quarter of an em below it, and a rendered text box carries about 1.12em of ascent, so the first x tick label clears the y axis's own bottom label only at about 1.42em below the axis, and the x axis title only at about 2.8em. Sizing these from the nominal font size instead leaves the origin corner's two labels ('0' and '0.00') overlapping at large type. Any figure reused at two type sizes must recompute them, and the check is a bounding-box intersection over every tick, axis title and note, not a visual glance.

Keep factual, functional, fit and design QA in external records tied to the final HTML hash. A successful build/check certifies reproducibility and freshness only.

Retained assembly reads its layout, title and theme from saved inputs. A conflicting legacy builder CLI override fails with a note to retain the choice in the model; it is never silently ignored. The `/presentify` intake writes the resolved choice into retained authoring inputs before invoking the builder.
