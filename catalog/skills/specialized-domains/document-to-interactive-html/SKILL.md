...
4. **Resolve the design direction, then brainstorm concrete tokens (creativity-first).** Do not jump straight to markup, and do not reuse a fixed house look (a recognizable style across runs is the templated "AI-generated" signature `[[hallmark-design]]` rejects). The goal each run is a UNIQUE, creative, interactive design - not a style mechanically derived from the document type. Resolve the direction in this order:
    ...
    - **Resolve the output aspect.** After the style direction is committed, resolve the output aspect (full-width / standard / portrait / other): a named `--layout` (or `using the layout <...>`) binds and skips the menu; otherwise offer the aspect menu mirroring the style menu; non-interactively, auto-pick by content (a deck or a PDF whose source carries `deck_like: true` -> full-width; a report / repository / text-dominant source -> standard). Record the resolved aspect (and whether it was auto-picked) in the design-record comment. See "Output aspect (the canvas)" in `references/interactive-features.md`.
