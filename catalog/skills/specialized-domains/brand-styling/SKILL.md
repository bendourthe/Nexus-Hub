...
- Vendor-specific brands the user does NOT own (Anthropic, OpenAI, Google, Tailwind, Material - DevAI-Hub policy is brand-neutral; the user must own the brand they apply)
...
If the user does not yet have a brand, this skill is still applicable: scaffold an empty default brand at `~/.devai-hub/brand/default/tokens.json` and have the user fill it as they make brand decisions. Do NOT silently invent brand tokens (a "professional navy and gray" is not a brand decision the agent makes for the user).
...
A brand lives entirely under `~/.devai-hub/brand/<brand-slug>/` after the user scaffolds it. Example layout for a brand named `acme`:

```
~/.devai-hub/brand/acme/
...
1. **Detect the user's brand state first**. Before picking any colors or fonts, ask the user (in one consolidated turn):
    ...
    3. If no brand exists, do they want to scaffold an empty default brand at `~/.devai-hub/brand/default/tokens.json` for them to fill?
    4. What is the artifact target? (deck / doc / pdf / web / multiple)
2. **Scaffold the brand directory**. Create `~/.devai-hub/brand/<brand-slug>/` and copy `templates/tokens.template.json` into it as `tokens.json`. If the user supplied brand values inline, fill them; otherwise leave the placeholders and tell the user which fields to fill in.
3. **Validate the tokens before consuming**. Required fields must all be present; hex colors must match `^#[0-9a-fA-F]{6}$`; font stacks must be non-empty strings; `spacing.base` must be a positive integer; `spacing.scale` must be an array of positive numbers. Missing required fields fail the load with a clear error: "Brand `<name>` is missing required field `palette.muted` - please add it to ~/.devai-hub/brand/<name>/tokens.json before continuing."
...
8. **For multiple brands, scope at the artifact level**. A user may have multiple brands (parent company + subsidiary, employer + side project). Each gets its own directory under `~/.devai-hub/brand/<slug>/`. The artifact selects which brand by name; the agent never mixes brand assets across artifacts.

## Common Rationalizations

| Rationalization | Reality |
|---|---|
| "The user wants 'professional' colors - I'll just pick navy and gray" | No. ASK the user for their brand tokens. If they have none, OFFER to scaffold an empty `~/.devai-hub/brand/default/tokens.json` for them to fill. The agent inventing brand decisions is the failure mode this skill exists to prevent. |
...
| "I'll inline the brand colors in this session and not persist them" | Inline-only brand decisions are session-bounded - the next session re-asks the user for the same values, or worse, the agent re-invents them. Persist to `~/.devai-hub/brand/<slug>/tokens.json` so the brand survives the conversation. |
...
- [ ] If the user has no brand, the skill offers to scaffold `~/.devai-hub/brand/default/tokens.json` (does not invent values).
...
- [ ] Multiple brands coexist under `~/.devai-hub/brand/<slug>/` without cross-pollution; each artifact selects exactly one brand.
...
- `templates/tokens.template.json` - schema-shaped placeholder with `null` and empty-string values for every field. The user copies this into `~/.devai-hub/brand/<brand-slug>/tokens.json` and fills the values.

The skill ships NO palette values, NO font URLs, NO logo files, NO voice rules. Anything else under `~/.devai-hub/brand/` comes from the user's own brand kit.
