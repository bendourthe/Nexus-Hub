...
Re-run the grounded-citation fact-check on the final body text (quotes still match the excerpt store; `[N]` claims still supported). New flags are fatal for handoff until the user accepts them.
