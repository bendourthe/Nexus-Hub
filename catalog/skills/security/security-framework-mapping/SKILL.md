...
description: Map a security skill, finding, detection, or control to its identifiers across MITRE ATT&CK, MITRE ATLAS, MITRE D3FEND, NIST CSF, and NIST AI RMF, then optionally tag the skill's frontmatter so a coverage matrix can be generated. Make sure to use this skill whenever the user asks "what ATT&CK technique is this", "tag this skill with the framework", "which NIST control covers X", "map this finding to ATT&CK / D3FEND / ATLAS", "build a framework coverage matrix", "what AI RMF measure applies here", or otherwise asks to align a security artifact with a published taxonomy, even when the framework name is implied (e.g., "what's the technique ID for credential dumping" implies ATT&CK). SKIP, do NOT use for, threat modeling a new system end-to-end (use architecture-design + a dedicated threat model), running an actual security audit (use run-security-audit), or generating compliance evidence for a specific regulation (use the per-regulation compliance skills like gdpr-compliance, soc2-compliance, iso27001-compliance).
summary_l0: "Map security skills and findings across MITRE ATT&CK, ATLAS, D3FEND, NIST CSF, and NIST AI RMF"
overview_l1: "This skill teaches the agent how to cross-map a security artifact (a skill, a finding, a detection, a control) to identifiers from five public taxonomies: MITRE ATT&CK (offensive technique catalog), MITRE ATLAS (adversarial machine learning), MITRE D3FEND (defensive countermeasures), NIST Cybersecurity Framework (Identify / Protect / Detect / Respond / Recover categories), and NIST AI Risk Management Framework (Govern / Map / Measure / Manage subcategories). It covers (a) which framework fits which kind of artifact, (b) how to find the right identifier in each taxonomy, (c) how to record the mapping in a Nexus-Hub skill's optional frontmatter fields (mitre_attack, atlas_techniques, d3fend_techniques, nist_csf, nist_ai_rmf) and a companion references/standards.md, and (d) how the optional tags enable the framework coverage-matrix generator. Trigger phrases: ATT&CK technique, D3FEND countermeasure, NIST CSF mapping, AI RMF measure, ATLAS technique, framework coverage matrix."
...
- Authoring or revising a security or compliance skill and you want to add the optional `mitre_attack` / `atlas_techniques` / `d3fend_techniques` / `nist_csf` / `nist_ai_rmf` frontmatter fields (see `AGENTS.md`, "Optional Security and Compliance Framework Mapping").
...
ATT&CK and D3FEND are paired: every defensive countermeasure in D3FEND maps to one or more ATT&CK techniques it counters. ATLAS is ATT&CK's sister catalog for ML systems. AI RMF is the governance lens NIST CSF lacks for AI-specific risk.
...
1. **Is this artifact about an attacker action, a defender action, or governance?**
    - Attacker action -> ATT&CK (IT) or ATLAS (ML).
