...
| `references/model-profiles.md` | GENERATED index of every per-model mirror, with its claim count and verification date. Regenerated by the writer on every write, so it never drifts from the index and no mirror is left as an orphan bundled file |
| `references/models/<model-id>.md` | One per-model profile mirror per profiled model, human-readable, generated from the index. Reached through the index above rather than listed individually here, which is what keeps this table stable as models are profiled |
