...
| A rendered artifact looks right | render it headless, then `geometric_audit.py <file>` and a capture of the edited region | A zero exit code from the audit AND a capture the agent has inspected. Re-reading the source you just wrote confirms the source, not the render; "I made the edit" is not evidence. Owner: `catalog/rules/html/visual-self-verification.md`. |
