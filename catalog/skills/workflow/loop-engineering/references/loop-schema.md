...
- Set `trace_log` to a JSON Lines sink (see the `trace_log` field above) so each iteration's observed events, tool calls, and `exit_reason` are recorded and the loop's decisions can be debugged and aggregated after the fact.

Two distinctions keep a `trace_log` honest about what it proves. Collapsing either turns a partial record into an apparent full one:

- **An available summary is not internal reasoning.** Where a model emits a human-readable account of why it chose something, that text is model output. It is not an authenticated record of an internal process, and storing it under a `reasoning` key implies access nobody has. Internal reasoning is unavailable; record it as unavailable rather than substituting the summary for it.
- **A host-observed event is not a self-attestation.** "The harness observed the process exit with code 0" and "the agent reported the command succeeded" have different trust properties. Keep them distinguishable in the record, because a self-attested claim that reads like an observation is how an unverified assertion acquires the authority of a measurement.

An absent iteration in the trace means the iteration was not recorded, which is not the same as the iteration not happening. Where coverage is partial, say so; a silent gap reads as a clean run. Field names and the wider span mapping are owned by `[[ai-agent-development]]` (`references/agent-span-contract.md`).
