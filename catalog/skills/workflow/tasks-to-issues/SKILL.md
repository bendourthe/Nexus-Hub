...
description: Convert the strict task checklist in a feature directory's tasks.md (or plan.md) into linked GitHub issues via the local gh CLI. Drives /tasks-to-issues end-to-end - parses every `- [ ] T### [P?] [US?] file_path` line, builds a per-task issue payload with labels (`nexus-hub`, `spec-kit-task`, `parallel`, `user-story-N`), and either dry-runs the gh invocations or executes them sequentially after user confirmation. Use whenever the user wants to convert tasks to issues, create GitHub issues from a plan, file issues from tasks.md, dry-run the conversion to preview gh invocations, fan a plan out to issue tracking, or drive sprint planning from a generated plan. Trigger phrases include 'tasks to issues', 'convert tasks to issues', 'create issues from plan', 'file issues from tasks.md', 'issue tracking from plan', 'gh issue create from tasks', 'fan out tasks to GitHub', 'sprint from plan'. Cross-links to `[[implementation-plan]]`, `[[cross-artifact-analyzer]]`, and `[[project-constitution]]`. SKIP: project boards (use gh directly), milestone management, label creation, free-form issue authoring, plans whose tasks do NOT match the strict `- [ ] T### [P?] [US?] file_path` format (re-run /generate-plan first), or any flow that needs more than one issue per task.
summary_l0: "Convert strict-format tasks.md / plan.md task lines into linked GitHub issues via the local gh CLI - dry-run or execute sequentially"
overview_l1: "This skill drives the /tasks-to-issues command end-to-end. It locates the active feature directory (positional argument, .specify/feature.json, or most recent plan under docs/<version>/plans/), parses every `- [ ] T### [P?] [US?] file_path` line against the strict regex from Phase 6 of docs/v2.1.0/plans/adoption-spec-kit.md, builds a per-task issue payload (title, body with file path + user-story link, labels `nexus-hub` + `spec-kit-task` + optional `parallel` + optional `user-story-N`), and either prints the resolved `gh issue create` invocations one per task (--dry-run) or invokes them sequentially after user confirmation. The skill enforces idempotency by rewriting each successfully-filed task line to append `[gh#<num>]`, so re-runs skip already-converted tasks. Pre-flight checks verify gh is installed and authenticated and that the working directory resolves to a GitHub repo. The mechanical parsing + gh invocation lives in the per-skill scripts/tasks-to-issues.sh (POSIX) and scripts/tasks-to-issues.ps1 (PowerShell) helpers; the references/gh-cli-auth-runbook.md provides a one-page runbook on gh auth setup-git and rate-limit handling. Read-only when --dry-run is set; otherwise the only side effects are issue creation in the user's GitHub repo and idempotency-marker rewrites in the local source file. Trigger phrases: tasks to issues, convert tasks to issues, create issues from plan, file issues from tasks.md, issue tracking from plan, gh issue create from tasks, fan out tasks to GitHub, sprint from plan."
---
...
3. **Labels**: comma-separated. Always include `nexus-hub` and `spec-kit-task`. Add `parallel` when `[P]` is present. Add `user-story-<n>` when `[US<n>]` is present.
