...
Detect the test runner, then run the tests in THIS phase's blast radius, not the whole suite. The blast radius is the test files covering the files Phase 2 added or modified, plus any suite the repository designates as its fast pre-commit gate (for Nexus-Hub: `python scripts/ci/run.py --profile fast`). Capture passed/failed/skipped for that scope. Route: 0 failures -> Phase 6 is a no-op; failures -> Phase 6.

**Scope per phase; the full suite and the coverage threshold belong to the final phase (9B).** A non-final phase measures whether IT broke something, which its own blast radius answers. Running the whole suite plus a coverage-augmentation round in every phase costs two full runs per phase (Phase 4 and again at 8.2) on a tree the plan itself calls incomplete, and drives test generation against interfaces later phases will still change. A six-phase plan paid twelve full-suite runs for a verdict the final phase has to re-establish anyway.

**Scoping is not deferring, and the distinction is what keeps this safe.** Every phase still ends green and attributable: a failure surfaces in the phase that caused it, while its diff is small and the 3-iteration budget in Phase 6 can actually reach it. Skipping tests entirely until the end would hand the final phase several phases of unverified code and failures it cannot attribute, which is the undebuggable diff `[[incremental-implementation]]` exists to prevent. If the blast radius cannot be determined for a change, run the full suite for that phase rather than guessing narrow.

## Phase 5: Test augmentation

Write tests for genuinely NEW behavior this phase introduced that no existing test reaches, and enforce project standards (AAA, parametrize, fixtures in `conftest.py`/setup, no `sleep`). Then proceed to Phase 6.

**Coverage-driven augmentation to the 80% threshold is a final-phase duty (9B), not a per-phase one.** Chasing a coverage number against a half-built interface produces tests that a later phase rewrites or deletes, so the work is paid twice and the second payment is a merge conflict against the agent's own output. A phase-scoped test proving the new behavior is durable; a test written only to move a percentage is not. Record what was left for 9B rather than expanding this step.
...
- **8.2 Test review (post-phase pass)** - re-run Phase 4's blast-radius command (not a second full-suite run); confirm every file added/modified in Phase 2 has at least one test referencing it; if gaps surface, run `/test` once for the unreferenced files, then log remaining gaps as `MT` in 8.5 (do not loop). The re-run exists because Phase 7's gate adjustments can drift the tree after Phase 4, and that green-then-red regression shows up inside the same blast radius that produced it, so the scoped command catches it at a fraction of the cost.
...
3. Check living docs architecture (`docs/handbooks/` mapped sources/outputs in legacy or topic layouts + code-grounded overview + technical companions, `docs/decisions/`, living `docs/README.md` / `DEVLOG.md` / `todos.md`). Self-gated: never invent `docs/testing/` or `docs/validation/`. Quote the check under `## Living docs architecture`.
...
**This is where the whole-suite obligations that non-final phases scoped away come due, and they are not optional here.** Phases 3-7 ran a blast-radius scope; this gate is the first and only point at which the plan's complete test surface is asserted. Run, in order: the FULL suite with coverage against the repository's canonical gate (for Nexus-Hub: `python scripts/ci/run.py --profile full`, never a hand-transcribed step list); enforcement of the project coverage threshold (>= 80%) with augmentation up to the 3-pass limit; and the cross-phase integration tests that no single phase's scope could have covered, because they exercise boundaries between phases. A phase-scoped green from Phase 7 is not evidence for any of the three. Quote the full-suite command and its result; a scoped run recorded here does not satisfy this gate.
