...
- **8.4 Known-gaps append** - via `[[known-gaps-tracker]]` (Append mode) into the correct `## v<MAJOR>.<MINOR>.<PATCH>` subsection of `docs/releases/v<MAJOR>/v<MAJOR>.<MINOR>/known-gaps.md`. Classify `# DEVIATION:` markers as NI/DF/BG; skipped failures as BG; coverage/untested-file gaps as MT; suppressed lint/warnings as WN; bypassed gates and unapproved CI gaps as QG. Every item carries Source phase, Plan reference, Reason, Suggested next step. Recompute the patch subsection's Summary; update Last updated; do NOT finalize.
