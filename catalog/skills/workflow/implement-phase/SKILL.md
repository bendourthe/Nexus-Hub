...
overview_l1: "The delegate behind /implement. It runs one plan phase end-to-end: resolve the phase and final-phase signals; review the plan against the codebase; implement each subtask in scope; lint, run the phase's blast-radius tests, troubleshoot, and apply a five-part GO/NO-GO gate including a proportional real-boundary smoke. The full suite, the coverage threshold, and cross-phase integration tests are asserted once, in the final phase. Phase 8 runs eleven ordered steps, records CI impact without editing pipelines, always writes the Plan delta, maintains gaps and release evidence, and ends every non-final phase commit-only. Phase 9 runs only on the final phase: reconcile architecture, gaps, and CI/CD; execute the Tier 3 deep pass; independently review the resulting tree against the plan Goal; pass the full local gate including the full suite, the coverage threshold, and cross-phase integration tests; then publish once with explicit approval and green integration checks. The ordered procedure lives in references/implement-phase-runbook.md. Trigger phrases include implement phase N, execute the plan, continue implementing, and implement full (alias in-full)."
---
...
- **Phase 9 - Final phase only.** Run the fail-closed last-phase duties, including duty 5's terminal pipeline reconciliation through `[[cicd-architect]]`, the Tier 3 whole-plan deep pass through `[[functional-verification]]`, and then an independent Goal-vs-codebase review of the resulting tree; write `<version_dir>/development/last-phase-evidence.md` with its own `## Tier 3 deep pass` section; resolve known gaps (9A); complete the LOCAL gate (9B); then run 9F: the final commit, explicit approval gate, plan's single branch push, integration pull request, required-check wait, and merge. The approval gate measures scope against the REMOTE integration branch (`git fetch`, then `origin/<base>..HEAD`), stating a commit count as well as a file count, because a branch cut from a local branch ahead of its remote carries those extra commits into the pull request and a local-only diff cannot see them. A red check REOPENS the phase and is reproduced locally before any re-push. Hand 9C-9E off to `/update release` only when the evidence file is complete, the deep pass and Goal-vs-codebase review have no unresolved miss without a recorded gap, AND the integration result is green and merged.
...
## Worktree isolation and parallel plans (v4.12.1)

Three guarantees, stated here because they change what a reader should expect at the start and end of a plan. The procedure lives in the runbook.

- **A plan runs in its own worktree, not just its own branch.** A branch alone serializes work: a second plan cannot start without switching the single checkout, and switching underneath a running session changes the files it is editing. Mechanics belong to `[[using-git-worktrees]]`; this skill owns only when a worktree is created and how it is named, so the release step can find it. Where worktrees are unavailable, fall back to a branch, say so in one line, and note that parallel plans are off for that session.
- **Phase 0 reports which other plans can start in parallel**, with a copy-paste launch block per plan, at the moment the user is deciding what to work on. Ranking and overlap detection belong to `[[plan-queue-assessment]]`; this skill owns the handoff and the launch instructions. If that skill is unavailable, mark the parallel report not covered rather than reconstructing its rules.
- **The final phase retires the worktree, after the merge is green and merged.** Verify the branch is fully merged and the tree is clean, then remove the worktree, delete the directory and the branch, and prune. Quote it under `## Worktree teardown`. Without this, ten plans leave ten stale checkouts behind; with it, a worktree is a temporary cost. Never remove one before the merge lands, and never `--force` past a dirty tree - both delete unfinished work.
...
| "Tests passed in Phase 4, I can skip the 8.2 re-run." | Phase 7's gate adjustments can drift the tree after Phase 4. The 8.2 re-run catches a green-then-red regression before the phase is declared done, and at blast-radius scope it is cheap. |
| "The phase is green, so the plan is tested." | Phases 3-7 run a blast-radius scope by design. The full suite, the >= 80% threshold, and the cross-phase integration tests are asserted once, in 9B. A scoped green is evidence about one phase, never about the plan. |
| "I should run the whole suite each phase to be safe." | That buys a verdict the final phase must re-establish anyway, at two full runs per phase, while generating coverage tests against interfaces later phases still change. Scope the phase; assert the whole in 9B. |
...
- [ ] The plan runs in its own worktree, created through `[[using-git-worktrees]]`, with its path recorded in the phase evidence; or a branch fallback was stated explicitly.
- [ ] Phase 0 reported parallel-capable queued plans via `[[plan-queue-assessment]]` with a launch block each, or stated that none qualify, or marked the report not covered.
- [ ] On the final phase, the worktree was retired only after a green merge: branch fully merged, tree clean, directory and branch deleted, `git worktree list` confirmed clear, quoted under `## Worktree teardown`.
- [ ] Code implemented subtask by subtask, in scope, with deviations logged.
- [ ] Lint clean; this phase's blast-radius tests ran; the five-part GO/NO-GO gate evaluated 0 failures in scope, no coverage regression on touched files, 0 lint errors, a successful build, and a cheap proportional real-boundary functional smoke whose expected and observed behavior matched through `[[functional-verification]]`, or the bypass was documented as a gap.
