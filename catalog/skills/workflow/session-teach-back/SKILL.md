...
description: Socratic mastery-confirmation loop that quizzes the HUMAN operator on what a development session actually built and why, one concept at a time against a persistent dated checklist, and refuses to finish until every item is confirmed. Make sure to use this skill whenever the user says "teach me what we built", "quiz me on this session", "teach-back", "did I understand X", "drill me on the session", "help me teach this to someone", "put me in teaching mode", "make sure I actually get what we did", "test my understanding of this work", or otherwise wants to confirm THEIR OWN understanding of a completed session (or guide someone else through it) instead of just recording it. SKIP, do NOT use for, generating a record of what happened (use generate-session-history / session-history), querying or recovering context from PAST sessions (use session-query), or tracking task or phase progress (use dev-progress-tracker).
...
- The user wants to teach what the session built to a named person ("help me teach this to <name>", "teaching mode") and wants a structured guide for what to explain and how to confirm that person understood.
...
- Keep the `mode` (`solo` or `teaching`) / `source` / `started` frontmatter and the `Progress: n/total confirmed` line at the top so any later exchange can resume from file state alone. In teaching mode, set `mode: teaching` and add a `student: <name>` field (see the teach-someone-else section below).
...
4. **Ask ONE targeted question** about that item, open-ended or multiple choice. Drill into the WHY behind the decision, not just the what ("why was that approach chosen over the alternative?", not only "what was built?"). When you pose a multiple-choice question, vary the position of the correct answer across questions (do not always make it "C"), and never reveal which option is correct until after the user has answered.
...
8. **Keep each exchange concise and depth-adjustable.** Aim for under ~2000 characters per turn. On request, pitch the explanation at `eli5` (explain like I'm 5), `eli14`, or `intern` level - simpler framing and analogies without dropping the actual decision being confirmed.

### 4. Teach-someone-else mode (optional)

When the user wants to teach what the session built to another person rather than self-quiz ("help me teach this to <name>", "teaching mode"), reuse the SAME mastery checklist but reframe it as a teaching guide. Set `mode: teaching` and `student: <name>` in the checklist frontmatter. Trigger this by phrase - Nexus-Hub skills do not take a `--student` argv flag.

In this mode, for each checklist section:

1. Suggest a concrete strategy for explaining that concept to the student (an analogy, a worked example, the order to introduce the ideas in).
2. Propose the specific questions the user should pose to the student to check that the concept landed.
3. Track what the user has explicitly covered and confirmed the student understood: mark an item `[x]` only once the user reports the student demonstrated it, and update the `Progress: n/total confirmed` line the same way as the solo loop.

The pedagogy is identical - one concept at a time, drill into the WHY, never batch, depth-adjustable on request - but the person being confirmed is the student, with the user as the teacher. The hard completion gate below still applies: the guide is not done until every item is confirmed understood.

### 5. Hard completion gate

Do NOT offer to wrap up, summarize, or declare the teach-back done until 100% of the checklist items are `[x]` and the `Progress` line reads `n/n confirmed`. An unfinished checklist means unconfirmed understanding; finishing early defeats the entire purpose of the skill. When every item is confirmed, state that mastery is confirmed and stop, then (and only then) consider the opt-in commit step below.

### 6. Opt-in: commit the mastery checklist (off by default)

The default behavior is to write the checklist locally and stop - do not commit or push it unprompted. After the teach-back (or teaching guide) completes, you MAY OFFER to commit the local checklist with a suggested conventional message, for example `data(teaching): add <slug> mastery checklist`. Only stage, commit, or push after the user explicitly confirms.

This is deliberately opt-in: an unprompted commit or push is an outward-facing git action that the global "destructive/outward-facing git requires confirmation" rule and the `git-guardrails` hook both forbid without explicit authorization. Never auto-push the checklist. If the user declines, leave the file on disk and do not raise committing again.
...
| "I'll just auto-push the checklist to save the user a step" | An unprompted `git push` is exactly the outward-facing action the `git-guardrails` hook and the global outward-git-requires-confirmation rule forbid. The commit is opt-in and off by default: write the checklist locally, and only commit or push after the user explicitly says to. |
...
- [ ] In teaching mode, the checklist used `mode: teaching` with a `student` field, and items were confirmed only after the student demonstrated understanding.
- [ ] Any multiple-choice question varied the correct-answer position and did not reveal the answer before the user responded.
- [ ] The checklist was committed only after explicit user confirmation, and was never auto-committed or auto-pushed.
