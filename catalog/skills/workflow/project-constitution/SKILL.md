...
summary_l0: "Author and maintain a versioned project constitution that declares MUST / SHOULD principles every plan, spec, and implementation aligns with"
overview_l1: "This skill produces and amends a project constitution at `docs/<version>/constitution.md` (the recommended Nexus-Hub-aligned location) or `CONSTITUTION.md` at the repo root. A constitution is distinct from CLAUDE.md / AGENTS.md (which are agent-instructions for AI tools): the constitution is project-governance for humans and machines alike, listing the MUST and SHOULD principles that every downstream artifact (specs, plans, ADRs, code) must align with. The skill defines the constitution structure (preamble, numbered principles, governance section, ratified / last-amended date line), the amendment workflow (detect old version, increment per SemVer - MAJOR for principle removals / redefinitions, MINOR for principle additions, PATCH for clarifications / wording fixes), and the Sync Impact Report emitted as an HTML comment at the top of the file (version change, modified principles, added or removed sections, downstream templates requiring updates). It is paired with the `/constitution` command (interactive author + amender) and the `catalog/templates/constitution-template.md` skeleton. Use it when establishing project principles for the first time, amending an existing constitution, or running a propagation check after constitution edits. Trigger phrases: draft a constitution, ratify principles, project governance, MUST rules, amend the constitution, set the ground rules, constitution check, what are our non-negotiables."
---
