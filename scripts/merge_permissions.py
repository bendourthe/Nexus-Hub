...
``scripts/installer.sh`` merges permission templates with ``jq``. macOS does not ship
...
and had no such dependency. That asymmetry is what this helper removes: the bash
installer keeps ``jq`` as its fast path when present and falls back here otherwise, so
both installers produce the same result on every host.
...
Two behaviors matter for parity with the ``jq`` path:
...
Prints the number of newly-added entries to stdout so the caller can report it.
"""
...
def merge(
    ...
    # Atomic write: a truncated settings.json would break the user's agent entirely,
    # and an interrupted installer is a realistic way to produce one.
    tmp_path = settings_path.with_suffix(settings_path.suffix + ".tmp")
    tmp_path.write_text(_dump(existing_doc), encoding="utf-8")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--template", required=True, type=Path,
                        help="Nexus-Hub permission template to merge FROM.")
