<#
.SYNOPSIS
    Install, update, or remove Nexus-Hub auto-approve permission configs for AI coding platforms.
...
.PARAMETER Uninstall
    Remove Nexus-Hub permission entries from each platform's config.
...
.EXAMPLE
    .\Install-Nexus-Hub-Permissions.ps1 -DryRun

.EXAMPLE
    .\Install-Nexus-Hub-Permissions.ps1 -Platforms CLAUDE,GEMINI

.EXAMPLE
    .\Install-Nexus-Hub-Permissions.ps1 -Uninstall
#>
...
function Uninstall-ClaudePermissions {
    ...
    if ($existingJson.permissions -and $existingJson.permissions.allow) {
        ...
        if ($PSCmdlet.ShouldProcess($settingsFile, "Remove $removedCount Nexus-Hub permission entries")) {
...
function Uninstall-GeminiPermissions {
    ...
    if ($existingJson.tools -and $existingJson.tools.allowed) {
        ...
        if ($PSCmdlet.ShouldProcess($settingsFile, "Remove Nexus-Hub tool entries")) {
    ...
    if ($existingJson.allowedDomains) {
        ...
        if ($PSCmdlet.ShouldProcess($settingsFile, "Remove Nexus-Hub domain entries")) {
    ...
    if ($PSCmdlet.ShouldProcess($settingsFile, "Write cleaned settings")) {
        $existingJson | ConvertTo-Json -Depth 10 | Set-Content $settingsFile -Encoding UTF8
        Write-Status "  Removed Nexus-Hub entries from settings.json" "Green"
    }
}
...
function Uninstall-CodexPermissions {
    ...
    if ($backup) {
        ...
        Write-Status "  No backup found. Remove Nexus-Hub sections from $configFile manually." "Yellow"
    }
}
...
Write-Host "       Nexus-Hub Auto-Approve Permissions Manager" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""
...
if ($Uninstall) {
    Write-Host "Mode: UNINSTALL (removing Nexus-Hub permission entries)" -ForegroundColor Yellow
    Write-Host ""
}
else {
