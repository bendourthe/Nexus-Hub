...
**Restore skill loading + presentify upfront design questions (v3.14.3).** Fixes the two independent root causes that made `/presentify` fail with "Unknown skill: document-to-interactive-html" on a normal install, then reworks `/presentify` to ask its four high-level design questions up front. Phase 0 (below) restores skill loading: it corrects invalid YAML frontmatter across 47 skills, adds a strict-YAML validator gate so the defect cannot regress, and switches both installers to flatten skills for Claude into the discoverable one-level layout.
