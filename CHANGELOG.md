...
**v3.15.5 model-prompting-research (IN PROGRESS, Phase 1 of 6).** A research-and-tune capability that keeps the catalog current as new models ship: a web-research swarm learns each live-enumerated model's current prompting guidance, verifies every claim against a primary source, records model-specific guidance in a distributed per-model profile layer, and auto-applies only model-agnostic authoring improvements to shared bodies behind the full guard suite on an isolated branch. Phase 1 lands the data spine and its two guards. No new outbound dependency or credential (web access is the agent's own `WebSearch` / `WebFetch`, so the MCP Registry Policy is not engaged). Catalog counts unchanged so far: **268 skills**, **16 commands**, **28 hooks** (the new skill registers in Phase 4).
