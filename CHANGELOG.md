...
**Selective Claude-Red offensive-methodology adoption (scope-gated, defensive-enrichment).** Adopts a small, re-authored slice of offensive-security methodology that sharpens Nexus-Hub's existing defensive review surface, gated behind a precisely-scoped `nexus-skill-scanner` allowlist and an Ask-First category decision (plan: [`docs/v3.1.0/plans/adoption-claude-red.md`](docs/v3.1.0/plans/adoption-claude-red.md); source comparison: [`docs/v3.1.0/comparison-claude-red.md`](docs/v3.1.0/comparison-claude-red.md)). All content is `skill-native` (pure Markdown methodology; zero new outbound calls, credentials, dependencies, or processors); the gating axis is scope / brand / dual-use, not network trust. Every adopted artifact is re-authored generically per the Reverse-Engineering Attribution Rule (no references to the external source, its maintainer, or the upstream checklist) and carries authorized-engagement preconditions in its Verification section. This is part of the v3.1.0 minor bump and shares the `[Unreleased]` section with the parallel dynamic-workflows adoption; both reconcile at develop-integration. Open items are tracked in [`docs/v3.1.0/known-gaps.md`](docs/v3.1.0/known-gaps.md).
...
- Catalog grows to **250 skills** (Security category 11 -> 13); the two new security skills are registered in `data/SKILL_INDEX.md`, `data/skills.json`, and the `data/marketplace.json` `plugin.description` headline count (248 -> 250).
