...
### Added
- **`/compare-project` now chains into `/generate-plan`.** A new Step 8 counts adoption items by tier (P0 / P1 / P2 / P3) and always asks whether to immediately generate an implementation plan, offering three scope choices (P0+P1, P0+P1+P2, or all items). On acceptance, `/compare-project` hands off to `/generate-plan` with the comparison file path and the selected scope filter, producing `docs/<version>/plans/adoption-<name>.md` in the same session. Declining prints a one-line pointer showing the exact manual-invocation command.
- **`/generate-plan` Step 0.5 — *From-comparison mode*.** When invoked with a single positional argument whose path matches `**/comparison-*.md` (either from `/compare-project` Step 8 or manually via `/generate-plan docs/<version>/comparison-<name>.md`), the command parses the report's Adoption Plan section, inherits the version from the comparison file's path, derives the slug as `adoption-<name>`, defaults the plan type to Feature/Enhancement, and skips the interview questions the report already answers (Goal, Scope In/Out, Affected Areas, Risk baseline). The abbreviated interview still asks Definition of Done, Testing Expectations, and any additional constraints the report did not capture. Phase design groups filtered adoption items by dependency order, mapping each item's `What | Source | Target | Effort | Risk` columns directly onto sub-task Objectives and Prompts.
- **`implementation-plan` skill v1.2.0** (`catalog/skills/workflow/implementation-plan/SKILL.md`) — frontmatter, When-to-Use, and overview updated to document the from-comparison hand-off path.

### Changed
- **`rag-implementation` skill retrofitted with `zilliztech/claude-context` patterns (Phase 1 of 6 of the v0.9.8 adoption plan).** Added a "Canonical OSS Reference" subsection after the existing RRF `hybrid_search` code block naming the upstream 8.4k-star MIT implementation and its SWE-bench token / tool-call reductions; extended the Vector Store Comparison table with **Milvus** and **Zilliz Cloud**; extended the Embedding Model Comparison table with `voyage-code-3` (VoyageAI), `gemini-embedding-001` (Google), and Ollama, plus a new "Code-Specialized Embeddings" subsection framing when to reach for code-specific embeddings; added an **AST-aware** row to the Chunking Strategy Comparison table with new "AST-Aware Chunking for Source Code" and "Incremental Re-Indexing with Content-Hash Merkle Trees" subsections citing the upstream `tree-sitter` splitter (9 languages) and the Merkle synchronizer. Skill grew from 1054 to 1077 lines (cap 1100). No registry, installer, or platform-template changes. Source: [docs/v0.9.7/plans/adoption-claude-context.md](docs/v0.9.7/plans/adoption-claude-context.md) Phase 1 (items A1 + A2).
- **`/compile-deep-research` pivoted from script-based to agent-driven.** The persistent generator at `scripts/compile_deep_research.py` has been deleted along with its entries in `scripts/installer.sh` / `scripts/installer.ps1`. The SKILL and command have been rewritten as a detailed playbook: per invocation the agent inspects the user-selected template's styles.xml / theme / header-footer, builds a style profile, synthesizes content, and writes a throwaway python-docx generator (`generate.py` saved in the cache dir for reproducibility) whose styling is derived entirely from the template. Fixes two v1 regressions: Word "unreadable content" warnings from brittle OOXML, and Phase 2 silently defaulting to the branded template instead of asking the user.
- **`/compile-deep-research` output layout split final outputs from intermediates.** Final `.docx` / `.pdf` / `.md` land in `<project>/docs/compiled/<ReportTitle>.{ext}`. Intermediate reproducibility artifacts (`merged.md`, `refs.json`, `style_profile.json`, `generate.py`, `ingest.json`) land in `<project>/.cache/compile-deep-research/<ReportTitle>/` with the per-title `<Title>_` filename prefix dropped. The command recommends gitignoring `.cache/` and offers to add the entry if the project's `.gitignore` does not already cover it.

### Removed (Breaking)
- **`/generate-implementation-plan` deprecation alias deleted.** The v0.9.7 forwarding shim (`catalog/commands/generate-implementation-plan.md`) has been removed along with every remaining textual reference that described the alias as preserved (in `catalog/commands/generate-plan.md`, `catalog/skills/workflow/implementation-plan/SKILL.md`, `README.md`, and `README_zh.md`). Users must now invoke `/generate-plan` directly. Historical `CHANGELOG`, `DEVLOG`, and release-notes entries are intentionally left as-is for version traceability.
