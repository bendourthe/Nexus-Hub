...
- **Interactive guide: copy-to-clipboard buttons + "Get started" rename** (`guides/interactive-guide/nexus-hub-guide.html`): runnable command examples (the Windows / macOS / Linux install commands) now carry a clear copy-to-clipboard button via a reusable `[data-copy]` injector (ready for future command blocks, including the planned v3.7.0 one-line bootstrap), the entry-point CTA "Get set up" was renamed to "Get started", and the guide's banner version label was refreshed to v3.6.0. Presentation-only (CSS + a small vanilla-JS injector); no new dependency.
