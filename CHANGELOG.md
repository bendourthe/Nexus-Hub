...
- **Catalog counts finalized to 259 skills / 25 hooks** (`AGENTS.md`, `README.md`, `.claude-plugin/plugin.json`, `data/marketplace.json`): the two new Security skills bring the catalog to **259 skills**; the two new advisory hooks bring it to **25 hooks**; commands stay **16** (`nexus-hub verify` is a CLI subcommand, not a Claude slash command). No further registry edit was needed for the Phase 3 and Phase 5 in-scope refinements (`competitive-generation` and `skill-stocktake` summaries were unchanged), consistent with the v3.8.0 / v3.9.0 refinement precedent. (adoption-ruflo Phase 6)
