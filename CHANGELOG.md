...
**v3.1.0 adoption (in progress): Dynamic Workflows residual (workflow-as-skill-bundle)**: the `adoption-dynamic-workflows` sub-plan operationalizes the workflow-as-skill-bundle distribution pattern (ship gracefully-degrading Dynamic-Workflow `.js` templates inside skill bundles, referenced from SKILL.md as templates to adapt) and pilots it on two high-value read-only fan-out skills, plus two minor orchestration enrichments. All content is skill-native (no new outbound call, credential, dependency, or third-party processor). Plan: [`docs/v3.1.0/plans/adoption-dynamic-workflows.md`](docs/v3.1.0/plans/adoption-dynamic-workflows.md). The version-carrying bump and tag are deferred to release time (`/update version`) once the sibling `adoption-claude-red` sub-plan also completes; see the master roadmap [`docs/v3.1.0/plans/v3.1.0-adoption-roadmap.md`](docs/v3.1.0/plans/v3.1.0-adoption-roadmap.md).
...
- `agent-orchestration-primitives` SKILL.md now pairs Dynamic Workflows with the Claude Code built-in `/loop` (interval / continuous runs) and `/goal` (hard completion requirement) commands, framed as platform commands to reference rather than catalog artifacts Nexus-Hub ships.
