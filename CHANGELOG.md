...
**v3.15.4 presentify-visual-fidelity (in progress).** Making the presentify skill's output visually faithful and self-correcting. Phases 1-5 land the full-width canvas contract, measurable image-sizing discipline, annotated-figure overlay recreation, reliable stock/mix imagery integration, and an iterative multi-agent visual-QA self-critique loop; Phases 6-7 (command/skill/registry polish and the refactor gate) follow. No version surface is bumped until release. Catalog counts unchanged: **268 skills**, **16 commands**, **28 hooks**.
