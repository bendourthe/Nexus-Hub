...
- **Claude Usage Monitor: richer usage-warning toast** (`extensions/claude-usage-monitor/src/recommendations.ts`, `src/extension.ts`): the usage-threshold warning notification is redesigned to echo the dashboard's richer style while staying an auto-dismissing toast. It now renders a `$(warning)` title carrying the metric and percent, the notification's built-in progress bar made determinate and filled to the usage percent (the closest native analog to a usage ring), and a detail line of per-recommendation codicon rows - `$(arrow-swap)` switch model, `$(dashboard)` reduce effort, `$(watch)` reset time. VS Code notifications cannot render a custom modal, so this maps the mockup onto the native `withProgress` toast (which stays self-dismissing and never stacks). The recommendation text stays model-aware and is taken from the shared `buildUsageSuggestion` structured parts (new `percent` / `label` / `resetsIn` / `switchModel` / `effortAdvice` fields) so the toast and the dashboard never drift. Extension 0.5.5 -> 0.6.0; no catalog change (265 skills, 16 commands, 25 hooks); no new outbound call, dependency, or credential. Prepared for a v3.11.1 patch release pending a visual confirm of the notification rendering.
