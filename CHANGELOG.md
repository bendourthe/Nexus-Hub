...
- **`test_file_size_budget` measured the developer's checkout instead of the artifact**, so it failed on every Windows checkout and passed in CI. It used `Path.stat().st_size`; with `core.autocrlf=true` a Windows working tree holds CRLF while the committed blob and every served copy hold LF, making the file one byte per line larger - 504222 on disk against a 499799 blob, a difference of 4423 that is exactly the file's 4423 lines. `git status` reported the file clean the whole time, which is what made a line-ending artifact read as a real budget breach. It now normalizes line endings before measuring, verified byte-identical to the committed blob and negative-controlled against a simulated over-budget artifact. Same defect class as the v3.15 BG-16 bootstrap test whose verdict depended on the launching shell. The budget itself was NOT raised: the guide has 201 bytes of headroom and that is recorded as a gap for a deliberate decision, not spent to turn a test green.
...
- **Archived v4.0 through v4.5 session history** (121 files), bringing per-version retention fully current for the first time since the policy shipped. v4.6 and v4.7 stay in the active tree, inside the two-minor threshold. Retention now reports a clean tree, and that result is trustworthy rather than vacuous: the same checker reported eight outstanding versions before these two passes ran.
- Reference repair for the v4 pass covered 65 links across 55 files. Rather than pattern-matching link shapes, each relative link was resolved against its original location and re-relativized to the same target, which is what correctly distinguished links into the version tree (`../../plans/`, `../last-phase-evidence.md`, `../guide-*/`) from depth-neutral links that escape `docs/releases/` entirely (`decisions/`, `tests/`) and had to stay byte-identical. Eight `docs/DEVLOG.md` links and one plan path citation were repointed.
- **Repaired seven stale live pointers in living documents** left by the layout refactor: the superseded-snapshot and deferred-work citations in `docs/policy/platform-read-contracts.md`, the residuals tracker in `docs/specs/antigravity.md`, the parity-migration tracker in `docs/specs/README.md`, the precedent citation in `docs/roadmap-prioritization.md`, and both carried-in plan references in the v4.8 known-gaps ledger, which the next plan reads forward. All seven now resolve.
- Deliberately NOT rewritten: the eight remaining `docs/v<N>/` mentions in `docs/roadmap-prioritization.md`. Each narrates a past state ("the moment `docs/v4/` existed, every plan under `docs/v3/` stopped being checked"), and retargeting them would make the file describe events that did not happen. A first pass did rewrite them by blanket regex and was reverted: that document is where the rule "live references are repaired, a record of what was true at the time is not" is stated. The same reasoning keeps the frozen `docs-cleanup-report.md` files, `docs/archives/**`, and decision records untouched.
