# Claude Code Instructions — DevAI-Hub
...
3. **New command** (`catalog/commands/<name>.md`): no registry update needed. If the command needs a style-guide reference, place it at `catalog/style-guides/<name>.md` (NOT in `catalog/commands/`, otherwise it surfaces as a slash command). The installer copies `catalog/style-guides/` to `~/.devai-hub/style-guides/`.
