...
| 2 | Model-specificity note + decline bookkeeping (skill-native, P3) | A one-line `model-routing` note (C4); a `drop-outright` matrix row for the imaging proxy citing the MCP Registry Policy and the ruflo precedent; CHANGELOG `[Unreleased]` entry; `docs/v3.11.0/known-gaps.md` revisit trigger; validator chain green | Moderate reasoning tier (Sonnet-class), low-to-medium effort (Claude Code: Sonnet 5, medium) |
...
**Prerequisites**: None. Source content is fully derived in [docs/v3.11.0/comparison-pxpipe.md](../comparison-pxpipe.md) Part A and Step 6.
