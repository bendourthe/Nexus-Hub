...
**Prerequisites**: None.
**Stability Gate**: `git status` shows only the intended new plan file under `docs/archive/v1/v1.0.0/plans/`; the abandoned plan carries an ABANDONED header that forward-links here.
...
**Prompt**:
> In `docs/v0.9.7/plans/adoption-claude-context.md`, prepend an `> STATUS: ABANDONED (2026-04-24) — SUPERSEDED BY v1.0.0 SECURITY-HARDENING PLAN` blockquote header that explains: (a) Phase 2 reverted; (b) Phase 1 content de-branded in v1.0.0 Phase 4; (c) Phases 3–5 reverse-engineered into v1.0.0 Phases 8–10; (d) Phase 6 absorbed into v1.0.0 Phase 11; (e) release retarget from v0.9.8 to v1.0.0. Include a forward-link to `docs/archive/v1/v1.0.0/plans/security-hardening-v100.md`. Leave the rest of the file untouched — it is historical record.
...
**Prompt**:
> Run `/generate-plan` in discovery mode with `~/.claude/plans/recursive-yawning-willow.md` as the authoritative source for every question. Slug: `security-hardening-v100`. Version: `v1.0.0`. Plan type: Feature/Enhancement. Expected output: `docs/archive/v1/v1.0.0/plans/security-hardening-v100.md` listing 12 phases (0 through 11) in the same order and with the same titles as the scratch plan's Phase Map. Confirm the abandoned-plan forward-link from sub-task 0.4 resolves to this output path.
...
- [x] `docs/archive/v1/v1.0.0/plans/security-hardening-v100.md` exists (this file)
...
**Goal**: Migrate `CHANGELOG.md`, bump 14 canonical files from `0.9.7 → 1.0.0` (skipping 0.9.8), author `docs/archive/v1/v1.0.0/RELEASE_NOTES.md`, update memory, create the git tag.
...
#### 11.3 — Author `docs/archive/v1/v1.0.0/RELEASE_NOTES.md`
...
**Prompt**:
> Create `docs/archive/v1/v1.0.0/RELEASE_NOTES.md` (~150–250 lines). Sections:
>
...
> 12. `docs/v0.9.7/plans/adoption-claude-context.md` forward-link resolves to `docs/archive/v1/v1.0.0/plans/security-hardening-v100.md`.
>
...
- [ ] `docs/archive/v1/v1.0.0/RELEASE_NOTES.md` authored
...
11. **Release artifacts**: `CHANGELOG.md` `[1.0.0]` populated; `docs/archive/v1/v1.0.0/RELEASE_NOTES.md` exists; 14 canonical files bumped; `MEMORY.md` has `project_release_v100`; `git tag v1.0.0` created locally.
