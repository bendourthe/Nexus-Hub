...
The v2.1.0 final release pulls **Phase 10 (G12 Integration Registry refactor)** forward from its original v2.2.0 target. The motivation is the user's request to expand supported platforms from the original 5 to 9+ in a single release, which the lock-step `base-*.md` editing convention could not absorb without 50+ correlated file edits. The ADR-001 architecture (`docs/archive/v2/v2.1.0/adr/adr-001-integration-registry.md`) introduces a Python class hierarchy under `scripts/lib/integrations/` -- `IntegrationBase` plus four mixin specializations (`MarkdownIntegration`, `TomlIntegration`, `YamlIntegration`, `SkillsIntegration`) -- with one subclass per supported platform.
