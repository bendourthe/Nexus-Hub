...
**Prior-version known-gaps ingest**: v1.1.5 is the immediately prior version; its `known-gaps.md` is finalized. The 5 still-open `DF-*` / `QG-001` / `MT-1` items concern cross-OS installer dry-run verification and are orthogonal to spec-kit adoption -- they are explicitly out of scope for this plan and remain tracked in `docs/archive/v1/v1.1.5/known-gaps.md` for a future CI-matrix plan.
...
| 9 | P3 polish - methodology essay, devcontainer, markdownlint config, installer test | `docs/archive/v2/v2.1.0/spec-driven-methodology.md`, `.devcontainer/`, `catalog/style-guides/markdownlint-cli2.jsonc`, `tests/installer/test_registrar_path_traversal.py` | v2.1.x patches |
...
**Prerequisites**: None.
**Stability Gate**: `/constitution` command runs end-to-end on a throwaway test project and produces `docs/archive/v2/v2.0.0/constitution.md`; all three target skills (`spec-driven-development`, `ambiguity-detector`, `idea-refine`) reference the new marker convention and the 3-marker hard limit in their bodies; `make validate` passes.
...
**Prompt**:
> Create three new artifacts implementing adoption candidate G1 from `docs/archive/v2/v2.0.0/comparison-spec-kit.md`:
>
...
**Stability Gate**: `make validate`, `make lint`, and `make test` all pass with zero new errors / warnings. `CHANGELOG.md` and `docs/archive/v2/v2.1.0/RELEASE_NOTES.md` author the v2.1.0 release. Cross-OS installer smoke verification recorded.
...
#### 8.2 -- Author CHANGELOG.md + docs/archive/v2/v2.1.0/RELEASE_NOTES.md entries
...
> 2. If a Linux or macOS host is available, repeat the smoke run with `bash scripts/installer.sh --target <throwaway-dir>` and record the result. If unavailable, record the deferral in `docs/archive/v2/v2.1.0/known-gaps.md` as a continuation of DF-003 / DF-005 / DF-006 from v1.1.5.
> 3. Write the smoke results to `docs/archive/v2/v2.1.0/installer-smoke-post.txt`.
...
- [ ] `docs/archive/v2/v2.1.0/RELEASE_NOTES.md` authored
- [ ] `data/marketplace.json` version bumped
- [ ] Cross-OS installer smoke run captured in `docs/archive/v2/v2.1.0/installer-smoke-post.txt`
...
#### 9.1 -- Author Spec-Driven Methodology essay at `docs/archive/v2/v2.1.0/spec-driven-methodology.md`
...
**Prompt**:
> Create `docs/archive/v2/v2.1.0/spec-driven-methodology.md` (a single-file essay, target 2500-3500 words). Sections:
>
...
> 2. `docs/archive/v2/v2.1.0/spec-driven-methodology.md` exists and is non-empty.
...
**Prompt**:
> Author `docs/archive/v2/v2.2.0/adr-001-integration-registry.md` (using the `architecture-decision-record` skill from the catalog) covering:
>
...
>
> Document the change in `docs/archive/v2/v2.2.0/installer-refactor.md` with a migration note (for downstream contributors who relied on editing the per-platform templates in lockstep -- they now edit the relevant base class or per-agent subclass instead).
...
> 5. Author `docs/archive/v2/v2.2.0/RELEASE_NOTES.md`.
...
- [ ] ADR-001 authored at `docs/archive/v2/v2.2.0/adr-001-integration-registry.md`
...
- [ ] `CHANGELOG.md [2.2.0]` and `docs/archive/v2/v2.2.0/RELEASE_NOTES.md` authored
...
1. **Reverse-Engineering Attribution Rule**: do NOT name "spec-kit", "GitHub Spec Kit", "github/spec-kit", or any specific upstream file path in user-facing Nexus-Hub artifacts (skill bodies, command bodies, README, docs). Attribution belongs in the comparison report (`docs/archive/v2/v2.0.0/comparison-spec-kit.md`) and in the reverse-engineering matrix at `docs/policy/mcp-reverse-engineering-matrix.md`, not in the distributed artifact. Use generic descriptive names ("spec template", "constitution skill", "cross-artifact analyzer") instead.
