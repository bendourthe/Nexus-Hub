...
6. **WN-8 cross-OS smoke**: ran the Windows smoke (installer -Help/-PrintConfig probes + the full pytest suite + eval) and recorded CI's ubuntu-latest run as empirical Linux test-suite evidence; re-deferred macOS + the Linux installer-probe/eval portion with a dated reason. Wrote `docs/archive/v2/v2.3.0/installer-smoke-post.txt`.

7. **Documentation**: updated `docs/archive/v2/v2.2.0/antigravity-cli-probe.md` (new Section 11 verification record, WN-2/3/4 marked resolved-via-docs with residuals), `docs/archive/v2/v2.3.0/known-gaps.md` (WN-2/3/4/8 -> Resolved, two new residual gaps), DEVLOG, README.
...
- Windows: 936 pytest cases pass (catalog/hooks 392+3 skipped, integrations+installer 260, validators 44, skill-server 43, code-search 168+1 skipped, web-fetch 29); eval recall 100% / precision 96.2% (reproduces `docs/archive/v2/v2.3.0/eval-baseline.md`); installer -Help/-PrintConfig probes exit 0 (negative case exits 1).
