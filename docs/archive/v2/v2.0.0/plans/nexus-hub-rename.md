...
**Prerequisites**: None.
**Stability Gate**: `make validate` and `make lint` pass on `main` at the current `v1.4.0` head, the rename map is committed at `docs/archive/v2/v2.0.0/rename-map.md`, and the backward-compat decision (in-place installer migration, no shim or symlink) is recorded in the same file.
...
**Prompt**:
> You are starting Phase 1 of the rename plan at `docs/archive/v2/v2.0.0/plans/nexus-hub-rename.md`. The goal of this sub-task is to capture the **pre-rename baseline** so that after the rename we can prove nothing else regressed. Do the following:
>
> 1. Confirm the current branch is clean (`git status` shows no modifications). If dirty, stop and ask for instructions.
> 2. Run `python scripts/validate_skills.py --bundles-only` and capture the full output to `docs/archive/v2/v2.0.0/baselines/validate-skills-pre.txt`. On Windows the user's environment has `PYTHONUTF8=1` mismatch issues (carry-over WN-002); if `make validate` fails with cp1252 errors, fall back to direct `python` invocation with `set PYTHONUTF8=1` (PowerShell: `$env:PYTHONUTF8=1`) and document this in the baseline file.
> 3. Run `python -m pytest catalog/hooks/tests -q` and capture to `docs/archive/v2/v2.0.0/baselines/hook-tests-pre.txt`. Expected baseline per `docs/archive/v1/v1.3.0/known-gaps.md`: **366 passed, 3 skipped**.
> 4. For each of `extensions/devai-skill-server`, `extensions/devai-code-search`, `extensions/devai-web-fetch`, run the project's test command (see the per-extension `pyproject.toml` for the entry point - typically `python -m pytest -q`) and capture to `docs/archive/v2/v2.0.0/baselines/extension-tests-pre.txt`. Expected per `docs/archive/v1/v1.3.0/known-gaps.md`: **37 + 36(1s) + 23 passed**.
...
**Prompt**:
> Build the authoritative rename inventory at `docs/archive/v2/v2.0.0/rename-inventory.md`. Required sections:
>
...
**Prompt**:
> Write `docs/archive/v2/v2.0.0/rename-decisions.md` covering:
>
...
>
> 1. `docs/archive/v2/v2.0.0/baselines/validate-skills-pre.txt`, `hook-tests-pre.txt`, and `extension-tests-pre.txt` exist and show green baselines (allowing the documented WN-001 / WN-002 carry-overs).
> 2. `docs/archive/v2/v2.0.0/rename-inventory.md` exists, lists every directory/file rename, the variant table, the reference count, and the asset-transfer line for `nexus_primary.png`.
> 3. `docs/archive/v2/v2.0.0/rename-decisions.md` exists and locks the canonical name forms, repo URL, installed root, MCP prefix, backward-compat policy, and version-bump rationale.
...
- [ ] Pre-rename validator baselines captured under `docs/archive/v2/v2.0.0/baselines/`
- [ ] Rename inventory committed at `docs/archive/v2/v2.0.0/rename-inventory.md`
- [ ] Naming canon and backward-compat decision committed at `docs/archive/v2/v2.0.0/rename-decisions.md`
- [ ] Working tree clean
...
>
> Approach: perform a textual find/replace using the variant table from `docs/archive/v2/v2.0.0/rename-inventory.md`, then re-run `make build-catalog` (if the Makefile target exists and works on the current OS) to verify the registries can still be regenerated cleanly.
>
> Steps:
>
...
**Prompt**:
> Apply variant replacements from `docs/archive/v2/v2.0.0/rename-inventory.md` to:
>
> - `.pr_agent.toml`
...
>
> Validation: `bash scripts/installer.sh --version` (if `--version` is supported, otherwise just `--help`) should print the banner. `pwsh scripts/installer.ps1 -Version` should do the same on Windows. Capture screenshots or terminal text into `docs/archive/v2/v2.0.0/installer-banner-preview.md` for the record.
>
...
**Prompt**:
> Per the backward-compat policy locked in `docs/archive/v2/v2.0.0/rename-decisions.md`, implement the migration logic at the very top of both installers, before any other work.
>
...
>
> Capture the smoke-run output to `docs/archive/v2/v2.0.0/installer-smoke-pre.txt`. If any failure surfaces, fix in a follow-up commit before advancing.
...
**Prompt**:
> Apply each variant from `docs/archive/v2/v2.0.0/rename-inventory.md` to every file under `catalog/`:
>
...
>
> Suggested implementation: write a Python script at `scripts/apply_rename.py` that walks `catalog/`, applies each variant in order, prints a per-file changed-line count, and writes a manifest to `docs/archive/v2/v2.0.0/rename-manifest.txt`. Make the script idempotent so it can be re-run safely.
>
...
>
> 6. **What's New in v2.0.0** (titled `## What's New in v2.0.0`): three subsections -- (a) the rename itself, listing the breaking changes; (b) the modernized installer with the ASCII banner and migration path; (c) the Nexus brand integration. Reference `CHANGELOG.md` and `docs/archive/v2/v2.0.0/RELEASE_NOTES.md` at the end.
>
...
> 4. Link audit: at minimum, the links to `https://github.com/bendourthe/Nexus-AI`, `CHANGELOG.md`, `docs/archive/v2/v2.0.0/RELEASE_NOTES.md`, `data/SKILL_INDEX.md`, and `LICENSE` must resolve to existing targets (the RELEASE_NOTES file is written in Phase 7 sub-task 7.5; until then a stub at that path is acceptable).
...
**Stability Gate**: `grep -rn "DevAI-Hub\|devai-hub\|devai_hub\|DEVAI_HUB" 2>/dev/null | grep -v "^./node_modules\|^./.git/\|^./docs/archive/\|^./docs/v[01]\\.\|^./CHANGELOG.md:" | wc -l` returns 0; `CHANGELOG.md` has a complete `## [2.0.0]` block listing every breaking change; `docs/archive/v2/v2.0.0/RELEASE_NOTES.md` exists.
...
>
> Output: a manifest at `docs/archive/v2/v2.0.0/documentation-sync-manifest.md` listing every file the skill touched and every file it flagged for manual review.
>
...
>
> Output: changes captured in a `docs/archive/v2/v2.0.0/config-sync-manifest.md`.
>
...
> 2. Add patterns for any v2.0.0-introduced artifacts (e.g. `docs/archive/v2/v2.0.0/baselines/*.txt` is intentional but heavy; keep or ignore at the maintainer's discretion).
> 3. Verify `assets/*.png` is not accidentally ignored.
> 4. Run the skill's full audit pass.
>
> Output: a `docs/archive/v2/v2.0.0/gitignore-audit.md` summary of the changes applied and any patterns flagged but not applied.
>
...
> - `### Carry-overs`: WN-001 (4 framework-specialist orphan-bundle warnings) and WN-002 (Windows `make`/`shellcheck` env workaround) -- their status as of v2.0.0 (open / closed / re-deferred), and a link to `docs/archive/v2/v2.0.0/known-gaps.md`.
>
> Create `docs/archive/v2/v2.0.0/RELEASE_NOTES.md` with:
>
...
> 6. Links to the CHANGELOG block, the migration commit, and `docs/archive/v2/v2.0.0/plans/nexus-hub-rename.md`
>
...
> 2. `CHANGELOG.md` contains `## [2.0.0]` and `docs/archive/v2/v2.0.0/RELEASE_NOTES.md` exists.
> 3. `docs/archive/v2/v2.0.0/documentation-sync-manifest.md`, `config-sync-manifest.md`, `gitignore-audit.md` exist.
...
- [ ] `docs/archive/v2/v2.0.0/RELEASE_NOTES.md` written
...
**Goal**: Run all validators one final time, address (or explicitly re-defer) the two carry-forward known-gaps from v1.3.0, bump every single-source-of-truth version literal from 1.4.0 to 2.0.0, write `docs/archive/v2/v2.0.0/known-gaps.md`, and cut the `v2.0.0` git tag.
...
**Prompt**:
> Run the full validation matrix and capture each output to `docs/archive/v2/v2.0.0/baselines/<file>-post.txt` (mirroring the pre-rename baselines from Phase 1 sub-task 1.1):
>
...
>
> Capture each smoke-run terminal output to `docs/archive/v2/v2.0.0/installer-smoke-post.txt`. If macOS or Linux is unavailable on the local machine, document the limitation in the file -- a follow-up CI smoke matrix is part of the open WN-002 carry-over, not a blocker for v2.0.0.
...
>
> Both items resolved. Write `docs/archive/v2/v2.0.0/known-gaps.md` with `Status: in-progress` (per the lifecycle: `/wrap-up-session` sweeps to in-progress, `/update-version` finalizes). Move WN-001 and WN-002 to a `## Resolved` table with `Resolved in: v2.0.0 Phase 8 sub-task 8.3` and a one-sentence note pointing at the fix commit. The Open Items section starts empty for v2.0.0.
>
...
>
> 1. All baselines under `docs/archive/v2/v2.0.0/baselines/` are green and diff'd against pre-rename baselines (no regressions).
> 2. Installer dry-runs in `docs/archive/v2/v2.0.0/installer-smoke-post.txt` show banner + migration working end-to-end.
> 3. `docs/archive/v2/v2.0.0/known-gaps.md` finalized; WN-001 and WN-002 closed.
...
- [ ] All validators green; post-baselines captured under `docs/archive/v2/v2.0.0/baselines/`
...
- [ ] `docs/archive/v2/v2.0.0/known-gaps.md` finalized
- [ ] Session history generated at `docs/archive/v2/v2.0.0/development/history/`
