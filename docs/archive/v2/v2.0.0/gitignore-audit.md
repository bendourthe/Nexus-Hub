...
| Path | Reason tracked |
|---|---|
| `docs/archive/v2/v2.0.0/baselines/*.txt` | Pre- and post-rename validator baselines. Tracked so the diff between them is reproducible at the v2.0.0 tag for auditors. They are small text files; size is not a concern. |
| `docs/archive/v2/v2.0.0/installer-smoke-pre.txt` and `installer-smoke-post.txt` | Installer dry-run captures. Same rationale. |
| `docs/archive/v2/v2.0.0/rename-manifest.txt` | Per-file change manifest from `scripts/apply_rename.py`. Tracked for audit traceability. |
