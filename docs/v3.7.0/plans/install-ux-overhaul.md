...
**Prompt**:
> Create `docs/v3.7.0/known-gaps.md` for any deferred items / warnings. Resolve open items, then run `/update release` for v3.7.0 (docs + devlog + gitignore + version bump 3.6.0 -> 3.7.0 + changelog + commit + tag + push, with its own confirmation gates). Never tag or push without confirmation.
