...
- **Personal-path leak in an unrelated doc.** `docs/v3.7.0/comparison-ralph-claude-code.md:265` contains `C:\Users\BEDOURTHE` (committed in `51081c8`, a separate comparison doc), which fails the `validate_no_personal_paths` gate. It must be redacted before the v3.7.0 release. NOT touched here -- every changed line traces to Phase 2.
