...
No new gaps introduced. `docs/v3.7.0/known-gaps.md` is created in Phase 5 (sub-task 5.2). Carried-forward constraints: WN-v36-1 (bash not fully runnable on the Windows dev host -- bash path verified on CI + a Phase 5 Mac smoke test) and WN-v33-1 (local `make`/ShellCheck not always on PATH -- validators run directly).
