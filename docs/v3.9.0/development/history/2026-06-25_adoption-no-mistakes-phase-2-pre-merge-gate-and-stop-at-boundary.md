...
- **Frontmatter deliberately untouched.** Per the plan, the registry/summary-edit decision is deferred to Phase 3 sub-task 3.4; Phase 2 changes only the bodies. CHANGELOG and `docs/v3.9.0/known-gaps.md` updates are likewise consolidated in Phase 3.
...
- `docs/v3.9.0/plans/adoption-no-mistakes.md` (Phase 2 exit checklist checked off)
- `docs/v3.9.0/development/history/2026-06-25_adoption-no-mistakes-phase-2-pre-merge-gate-and-stop-at-boundary.md` (this file)

## Next

Phase 3: PR body, optional session matching, and consolidation. Add the optional deterministic-PR-body-from-audit-trail pattern to `pr-description-writer` (N5); make and record the build-or-defer decision on the diff-to-session matching enrichment of `session-query` (N7, recommended default: defer); record the host-side runtime decline and the default-on-telemetry cautionary in `docs/policy/mcp-reverse-engineering-matrix.md`; make the registry-edit decision; and add the consolidated CHANGELOG `## [Unreleased]` entry and `docs/v3.9.0/known-gaps.md` updates for the whole cycle.
