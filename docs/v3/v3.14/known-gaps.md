...
Three pre-existing test failures surfaced on this branch; none touches any file this version changed:
...
- `tests/validators/test_scan_skill_security.py::test_catalog_gate_is_clean` (surfaced during the Phase 7 regression net) - the `scan_skill_security` catalog gate reports 1 HIGH finding: "class 2 Data Exfiltration - credential exfiltration (env read + network egress)" at `catalog/skills/specialized-domains/document-to-interactive-html/scripts/fetch_stock_media.py:481` (`os.environ.get("PEXELS_API_KEY")` + the Pexels API call). This is a FALSE POSITIVE - the file is a legitimate, consent-gated stock-media API client that reads its own user-provided key and calls its own API (the v3.14.3 bring-your-own-key feature); it is not credential harvesting. It is PRE-EXISTING and unrelated to v3.14.5: `git diff develop..HEAD` shows the file unchanged by this branch, and `git show v3.14.4:...fetch_stock_media.py` confirms the same read shipped at the v3.14.4 tag (added in v3.14.3 Phase 3, commit a023a7e). Because CI runs this same gate (`ci.yml` line ~106, `--fail-on high`), it is a RELEASE-READINESS caveat that predates this release: it needs maintainer adjudication (cap it like the `catalog/skills/security/` category cap, add a scanner allowlist entry for the consented BYO-key pattern, or tune the class-2 heuristic to exempt an env-read that is the client's OWN documented API key) - a security-gate decision owned by the maintainer, deliberately NOT changed inside this unrelated release. Tracked here so `/update release` and the maintainer can decide before/with the v3.14.5 tag.
