...
See [`docs/v3/v3.4/known-gaps.md`](../../known-gaps.md). Open: DF-v34-1 (now narrowed -- `switch-model.sh` is pytest-gated and CI ShellChecks skill scripts; residual is a direct unit test for `detect-platform`/`enumerate-models`), WN-v33-1 (local `make`/ShellCheck unavailable; validators run directly), WN-v33-2 (benign pre-existing global-audit warnings outside this work). No resolved items this phase.
