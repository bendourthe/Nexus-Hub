...
| BG-5: Windows PowerShell discards the autonomy hook payload on the hosted runner | Open, release-blocking | The first unconditional `Console.In` repair failed remotely; the raw standard-input handle variant passes 39 focused local tests and requires hosted confirmation |
