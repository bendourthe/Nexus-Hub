# Org Knowledge Layer - Deep Research (supporting v3.17.1)
...
v3.16.6 is current; `docs/v3/v3.17/` holds the unreleased v3.17.0 agent-autonomy-toggle plan, so **v3.17.1 is a free slot**. v3.17.0 Amendment A1 touches the `base-*.md` set; v3.17.1 must sequence on top of it. Plan format follows AGENTS.md's current model-routing contract (separate tier and effort columns plus a `## Current model map`), not v3.17.0's older single-column shape.
