...
- Four documentation-verified-but-not-live-verified residuals remain (the `.agent/` vs `.agents/` codelab dissent, the exact global subpath, the `subagents/` / `rules/` subdirs, and whether `agy` requires a root `AGENTS.md`); they are tracked in `docs/archives/v2/v2.2/antigravity-cli-probe.md` and v2.3.0 known-gaps WN-v23-5.

## Source of truth

Mirrors `scripts/lib/integrations/antigravity.py` (`Antigravity10Integration`, `Antigravity20Integration`), the `AGENTS.md` platform-coverage section, and `docs/archives/v2/v2.2/antigravity-cli-probe.md`.
