...
- **`docs/v3.10.0/known-gaps.md`** (edited): status advanced to Phase 4 complete; recorded `DF-v310-ruflo-P4-extensions` (the `extensions/` MCP-server sources are an intentional out-of-scope manifest boundary) and a note that the manifest is a release-time artifact, not committed mid-cycle.
...
- **Attribution grep**: zero matches in distributed artifacts for `ruflo` / `AIDefence` / `AgentDB` / `RuVector` / `SONA` / `ReasoningBank` / `MetaHarness` / `rvf` / `rvagent` / `ruv.io`. (`ruflo` appears in `docs/v3.10.0/known-gaps.md` and the DEVLOG only as the cycle/plan identifier, which the attribution rule permits for internal docs.)
...
- `docs/v3.10.0/known-gaps.md` (Phase 4 status; DF-v310-ruflo-P4-extensions; release-artifact note)
- `docs/v3.10.0/plans/adoption-ruflo.md` (Phase 4 exit checklist checked off)
- `docs/DEVLOG.md` (Phase 4 entry)
- `docs/v3.10.0/development/history/2026-06-30_adoption-ruflo-phase-4-nexus-hub-verify-supply-chain.md` (this file)
