...
**Status**: in-progress
**Last updated**: 2026-05-20 (Phase 4 close -- no new gaps, no resolved items; Phase 4 was a pure-additive landing of `catalog/templates/spec-template.md` (8 mandatory sections with the FR-### / SC-### ID conventions and P1/P2/P3 user-story discipline) plus three body-only updates to `catalog/skills/developer-experience/spec-driven-development/SKILL.md` (new "Spec template" and "User stories with priorities" subsections, two new Common Rationalizations rebuttals on bullet-vs-ID and single-story specs, and two new Related Skills cross-links to `cross-artifact-analyzer` and `project-constitution`); the Phase 4 coverage-matrix verification was run against a throwaway `scratch/` spec+plan+tasks triple, confirmed `/analyze-spec` Pass 5 would correctly identify FR-003 and SC-002 as zero-coverage with severity HIGH per the Step 4 heuristic, then scratch files were deleted; no DEVIATION markers, no test failures, no suppressed lint rules, no bypassed gates, and no data-registry edits were required because Phase 4 ships a template and an existing-skill body update -- no new skill)
