...
**Status**: in-progress
**Last updated**: 2026-05-20 (Phase 3 close -- no new gaps, no resolved items; Phase 3 was a pure-additive landing of the new `/analyze-spec` command and the `cross-artifact-analyzer` skill plus the three data-registry updates, with no DEVIATION markers, no test failures, no suppressed lint rules, and no bypassed gates; the plan file itself was relocated from `docs/v2.0.0/plans/` to `docs/v2.1.0/plans/` to align with the targeted release version, and the Plan link above was updated to follow)
