...
| MCP Registry Policy + RE matrix | `AGENTS.md` + `docs/v1.0.0/mcp-reverse-engineering-matrix.md` + 3 internal MCPs | Regulated-industry safe; no third-party data processors required |
...
This section gates Section 11. Reference: [`AGENTS.md`](../../AGENTS.md) MCP Registry Policy and [`docs/v1.0.0/mcp-reverse-engineering-matrix.md`](mcp-reverse-engineering-matrix.md).
...
- **Attribution stripping**: Per the reverse-engineering attribution rule, the ported skills must use generic descriptive names (already done in target paths above) and must not credit mattpocock in the user-facing artifact. Attribution belongs in `docs/v1.0.0/mcp-reverse-engineering-matrix.md` if a row applies (none of these are MCPs, so no row is required) or in this report.
