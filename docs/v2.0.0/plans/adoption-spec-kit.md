...
**Prior-version known-gaps ingest**: v1.1.5 is the immediately prior version; its `known-gaps.md` is finalized. The 5 still-open `DF-*` / `QG-001` / `MT-1` items concern cross-OS installer dry-run verification and are orthogonal to spec-kit adoption -- they are explicitly out of scope for this plan and remain tracked in `docs/v1.1.5/known-gaps.md` for a future CI-matrix plan.
...
1. **Reverse-Engineering Attribution Rule**: do NOT name "spec-kit", "GitHub Spec Kit", "github/spec-kit", or any specific upstream file path in user-facing Nexus-Hub artifacts (skill bodies, command bodies, README, docs). Attribution belongs in the comparison report (`docs/v2.0.0/comparison-spec-kit.md`) and in the reverse-engineering matrix at `docs/v1.0.0/mcp-reverse-engineering-matrix.md`, not in the distributed artifact. Use generic descriptive names ("spec template", "constitution skill", "cross-artifact analyzer") instead.
