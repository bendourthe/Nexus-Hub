...
Owned-file writing consults the installation manifest. It can repair an artifact created by Nexus-Hub while preserving a file authored by the user. Some derived catalog materialization paths synchronize on byte difference and track their output; do not assume every copy path has identical overwrite behavior. Both installers activate user-attribution Git hooks from a digest-checked copy that survives the installer checkout; source checks accept LF/CRLF differences, but content changes require reinstall. Run nexus-hub attribution check before commits, tags and pushes. New-branch pushes scan the actual destination's history, not other remotes or stale tracking refs. Human identity and existing hooks are preserved; direct hosting API writes need separate verification.
