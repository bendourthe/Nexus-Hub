...
Seeded in Phase 1 and re-stated here for completeness. `settings.json` documents `effortLevel` ("Set a default effort level for models without a saved level of their own"; the settings page now delegates the definition to `settings-reference` and the accepted values to `model-config`, which lists `low`, `medium`, `high`, `xhigh`, and `max` for Fable 5.1, Fable 5, Opus 5, Sonnet 5, Opus 4.8, and Opus 4.7). The same page documents `CLAUDE_CODE_EFFORT_LEVEL` as taking precedence over settings and `/effort`, accepting level names or `auto` but NOT `max`, so the two levers no longer share one value set; Nexus-Hub seeds `high` on both, which every documented model accepts. A new per-model `modelSettings` key stores the level `/effort` writes and is not seeded. Re-verified 2026-09-04 (the 2026-08-30 statement quoted "Accepts low, medium, high, or xhigh", which is no longer what the vendor page says), `model` (read once at session start), and `env` ("Environment variables applied to every session and to subprocesses Claude Code spawns from it"). The same page states that `CLAUDE_CODE_EFFORT_LEVEL` **overrides** `effortLevel` for a session, which is why `configs/platform-defaults.json` declares both together. Documented scopes: `~/.claude/settings.json`, `.claude/settings.json`, `.claude/settings.local.json`.
