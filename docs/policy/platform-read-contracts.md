...
**Scope limit, stated plainly**: this was NOT a full re-verification pass. Exactly one platform was fetched, the newly added `pi`. Every other row carries its status from the 2026-08-25 v3.21.0 stamp and gains no claim from this cycle.

Pi contract read from first-party documentation in the project's own repository: skills at `~/.pi/agent/skills/` and `.pi/skills/` ([skills.md](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/skills.md), which states Pi implements the [agentskills.io specification](https://agentskills.io/specification)); prompt templates at `~/.pi/agent/prompts/*.md` and `.pi/prompts/*.md`, filename becoming the command name ([prompt-templates.md](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/prompt-templates.md)); context files at `~/.pi/agent/AGENTS.md` plus cwd and ancestors ([usage.md](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/usage.md)); project trust and the settings lever ([settings.md](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/settings.md)).

Two findings shaped delivery rather than merely being recorded. Pi reads project-root `AGENTS.md`, which Codex already writes, so the Pi integration does NOT claim that file. And Pi gates project-local `.pi` resources behind a trust prompt, so workspace-scope writes are inert until the user trusts the folder; nothing is pre-trusted on their behalf.

Decision record: [`docs/decisions/implemented/policy/2026-08-25-support-pi-as-a-distribution-platform.md`](../decisions/implemented/policy/2026-08-25-support-pi-as-a-distribution-platform.md).
