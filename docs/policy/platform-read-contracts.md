...
**Last verified**: 2026-09-16 for v4.12.1. v4.12.1: official discovery documentation fetched on 2026-09-16 for claude, cursor, codex, antigravity2, copilot, gemini, gemini-cli, openclaw, qwen and windsurf; all ten MATCH their declared read-paths. Windsurf was initially misclassified DRIFT during this pass by comparing the vendor's RULES documentation against the contract's workspace_dir, which governs skills, workflows and hooks rather than rules; the integration already writes .devin/rules/nexus-hub.md alongside AGENTS.md and legacy .windsurfrules, and the vendor has not moved the skills, workflows or hooks surfaces off .windsurf. Windsurf is a rename in progress to Devin Desktop, not a discontinuation: docs.windsurf.com redirects to docs.devin.ai, the marketed name is Devin Desktop IDE, and windsurf persists in config paths such as ~/.codeium/windsurf/. Codex docs 308-redirect from developers.openai.com to learn.chatgpt.com (host move, paths unchanged). Antigravity documents workflow retirement on 2026-11-01; skills already take precedence and Nexus-Hub already emits every command as BOTH a skill and a workflow, so no re-architecture is required. Cursor now documents RECURSIVE skill discovery and additionally reads ~/.claude/skills and ~/.codex/skills; the flattened layout Nexus-Hub installs is still discovered. kimi and pi remain UNVERIFIED this cycle (no dedicated vendor discovery page; both follow the SKILL.md standard). Nexus-AI remains UNVERIFIED as before.
