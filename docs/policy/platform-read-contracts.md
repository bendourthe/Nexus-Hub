...
**Last verified**: 2026-09-21 for v4.13.0. v4.13.0: re-fetched this cycle from first-party vendor documentation on 2026-09-21 and classified MATCH: claude (~/.claude/skills/<name>/SKILL.md, plus documented upward traversal to repo root and lazy load below the start dir), cursor (~/.cursor/skills and .cursor/skills, RECURSIVE, and it also reads .claude/.codex skill dirs at both scopes), codex (now learn.chatgpt.com after the documented 308 from developers.openai.com; $HOME/.agents/skills, $CWD/.agents/skills upward to repo root, and /etc/codex/skills, one level deep, folder-per-skill), antigravity2 (workspace .agents/skills/<name>/SKILL.md and .agents/workflows/<name>.md; global ~/.gemini/config/skills and ~/.gemini/config/workflows; workflow retirement still dated 2026-11-01 and skills still take precedence), and copilot (workspace .github/skills, .claude/skills, .agents/skills; user ~/.copilot/skills, ~/.claude/skills, ~/.agents/skills). CARRIED FORWARD from the 2026-09-16 v4.12.1 pass, not re-fetched this cycle: gemini, gemini-cli, openclaw, qwen and windsurf, all MATCH at that date. kimi, pi and nexus-ai remain UNVERIFIED as before (no dedicated vendor discovery page; kimi and pi follow the SKILL.md standard, nexus-ai is a private source). One observation worth recording although it is not drift: Copilot's USER scope now documents ~/.claude/skills, which Nexus-Hub already populates, so Copilot surfaces the catalog's skills globally without the opt-in .github/skills wrapper set. The wrapper set remains the only way to get a COMMIT-VISIBLE project surface. scripts/verify_platform_contracts.py reported OK for all 14 platforms against this JSON. No unexpected redirect to a new host was observed on any page fetched this cycle.
...
| Antigravity `agy` CLI (`antigravity2`) | global | shared `~/.gemini/GEMINI.md` | UNVERIFIED; not emitted | flattened `~/.gemini/antigravity-cli/skills/<name>/` | UNVERIFIED; not emitted | shared `~/.gemini/GEMINI.md` | shared `~/.gemini/config/hooks.json` + `~/.gemini/config/hooks/` (source-qualified 2026-09-24) |
...
| Cursor (`cursor`) | global | Attribution policy via `sessionStart` additional context | `~/.cursor/commands/<name>.md` compatibility write, UNVERIFIED; commands also emit as skills | flattened `~/.cursor/skills/<name>/` (+ command-skills) | `~/.cursor/agents/*.md` | none | `~/.cursor/hooks.json` + `~/.cursor/hooks/` (git-guardrails and attribution context) |
...
The 2026-08-30 audit, narrowed by the targeted 2026-09-24 recheck, leaves the following contract gaps open. They are deliberately excluded from MATCH claims and enforced install checks:

1. Antigravity CLI loose workflow and agent directories remain unverified. Its global hooks read the shared `~/.gemini/config/hooks.json` path already emitted by Nexus-Hub; no separate CLI-only hook directory is claimed.
2. Cursor legacy `.cursor/commands` and `~/.cursor/commands` discovery remain undocumented in the current official skills reference. Compatibility writes remain, while every command is also available through the verified command-skill path.
...
5. Nexus-AI's current public source qualifies the global `~/.nexus-ai/catalog/skills` reader. The optional project-local mirror and other mirrored surfaces were not traced in this targeted pass and remain UNVERIFIED.
