...
>
> For each row, Grep the proposed category directory for the slug to confirm no collision. If any collision is found, propose a `-doc` or `-template` suffix and document the rename. Output a short markdown table with columns: `Skill | Final category | Final slug | Collision check` and append it to this plan file as an inline comment block (or a small note file at `docs/v1.3.0/plans/audit-category-placement.md`). Do NOT create any SKILL.md files yet -- this sub-task only resolves placement decisions.
...
> 5. Where this went: `docs/v1.3.0/comparison-pm-claude-skills.md` and `docs/v1.3.0/plans/adoption-pm-claude-skills.md`, with v1.4.0 as the target release.
>
