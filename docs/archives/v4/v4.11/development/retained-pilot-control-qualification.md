...
**Publication addendum, 2026-09-25 UTC**: PR #298 passed 21 hosted checks with one expected skip and merged as `1ae58ad5`. Post-merge run 36097633753 passed smoke and provenance on that commit. The publication condition in the preceding paragraph is satisfied, and MT-9 is closed within the declared-control and retained-artifact boundary; the whole-page `status=fail` and future `unchecked` controls remain separate.
