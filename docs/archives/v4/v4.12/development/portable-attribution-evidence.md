...
All 17 instruction templates include the same mandatory preflight and shared attribution guide. New work must use the configured user, with no agent identity, coauthor trailer or agent footer. Claude's documented empty commit/PR attribution settings are seeded through the canonical defaults and generated settings. Cursor receives the policy through sessionStart additional context, using the installer's resolved Python executable. Aider receives the policy read path, five disabled attribution flags and enabled Git verification. Aider conflicts and preserved Cursor hooks receive NEEDS SETUP notes; existing Claude settings follow the normal preservation policy. The detailed policy lives once in the distributed style guide; the five locked template word budgets increased by exactly 30 words for the new preflight section.
...
At `438482bc`, the native macOS rerun passed all 39 attribution cases in 103.41 seconds. The Linux test runner also has PowerShell installed, exposing an incorrect test-matrix assumption: it tried both Windows-installer cases on Linux and failed at the existing Windows-only robocopy dependency. The fixture now selects PowerShell installer cases only on Windows and retains Bash coverage on POSIX hosts. The original Linux log is retained as `pr217-linux-tests-first.log`; no native coverage, installer behavior or timeout is removed or relaxed.

The Windows regression job passed on `438482bc`. A Linux scratch-container check with an executable pwsh sentinel on PATH confirmed that the corrected fixture skips precisely the two Windows-only cases (39 other cases deselected) without invoking the sentinel. This is matrix-selection evidence, not a replacement for native installer execution. The corrected PR head must still pass all required jobs.

The [known gaps](../../../../releases/v4/v4.12/known-gaps.md) retain GitHub PR-ref limitations, the stale Code contributor display, incomplete historical adversarial review and the earlier Windows whole-profile timeout. None is silently closed by portable enforcement. The existing CI lint/reporting and native authoring gaps retain their owners. The user operation and recovery procedure is in the [attribution guide](../../../../guides/user-attribution.md).
