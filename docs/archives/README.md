...
| **v4** *(current major, partial)* | `development/history/` subtrees v4.0-v4.5 and v4.7-v4.12, the v4.1 handbook snapshot, v4.0 CI task reconciliation and extension-suite qualification, bounded v4.3 platform-source and GitHub-settings evidence, v4.4 Training layout and platform-mark disposition, v4.8 Windows hook qualification, v4.11 temporal-value evidence, and v4.13 follow-up evidence | 377 | Closed session histories aged out under the one-minor retention threshold; the aborted v4.13 trigger-pilot attempt is frozen separately while its open bug remains in `docs/releases/v4/`. |
