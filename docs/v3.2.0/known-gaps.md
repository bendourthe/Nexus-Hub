...
**Status**: v3.2.0 `adoption-teach` in development on `feat/adoption-teach` (off `origin/develop`). All 3 of 3 phases complete (Phase 1: core `session-teach-back` skill + registration; Phase 2: teach-someone-else mode, eli5/eli14/intern depth levels, multiple-choice discipline, and an opt-in off-by-default checklist commit added to the same skill body; Phase 3: bidirectional `[[session-teach-back]]` Related-Skills back-links across the `session-*` family, a `using-nexus-hub` task-table mention, and a CHANGELOG `[Unreleased]` entry). Catalog green at Phase 3: JSON catalogs valid (251 skills -- Phase 3 adds only Markdown cross-links and a CHANGELOG entry, no new skill), the skill-security scanner gate is clean (0 HIGH/CRITICAL, exit 0), and `validate_skills.py --bundles-only` / `--quality` / no-personal-paths / unicode-safety / supply-chain / workflow-security / solution-frontmatter / `check_version_sync.py` all exit 0. The v3.2.0 version bump, CHANGELOG finalization, count-prose reconciliation, and tag are applied at the develop->main release (owned by `/update version` / `/update release`).
**Last updated**: 2026-06-09 (Phase 3)
