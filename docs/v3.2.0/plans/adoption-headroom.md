...
**Prompt**:
> Expand the package README and add a short architecture note under `docs/v3.2.0/` covering: the ContentRouter -> strategy pipeline, CCR reversible retrieval, the local-first/zero-outbound guarantee, the optional ML module's offline-first design, and the rtk migration path. Follow `catalog/style-guides/markdown.md`.
