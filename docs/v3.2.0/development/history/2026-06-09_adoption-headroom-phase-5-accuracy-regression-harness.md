...
2. **T017 -- wire the gate**. Three surfaces: a `make compress-eval` target (writes a report to `docs/v3.2.0/compression-eval-baseline.md` and runs the gate), a step inside the `make validate` recipe, and a step in the CI `validate` job (`.github/workflows/ci.yml`) immediately after the skill-security gate. The CI step runs `cd extensions/nexus-context-compressor && python -m evals --check` -- no `pip install` needed because the runner self-bootstraps. The gate stays quiet in pure `--check` mode (one-line PASS/FAIL to stderr) to respect the repo's output-minimization convention. Documented the threshold and the intentional re-baseline procedure in `evals/README.md`.
