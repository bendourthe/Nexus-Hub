...
**Created**: 2026-06-16
**Source comparison**: [docs/v3.6.0/comparison-spec-kit.md](../comparison-spec-kit.md)
...
No constitution file found at docs/v3.6.0/constitution.md (nor at docs/v3.5.0/constitution.md or docs/constitution.md) - skipping check. Recommend running `/constitution` to establish project principles. This is informational, not blocking.
...
**Prompt**:
> In the Nexus-Hub repo, update two skill bodies to document three workflow-control patterns adopted from GitHub Spec Kit's workflow engine (see docs/v3.6.0/comparison-spec-kit.md candidate N2a and Section 3 row 2):
...
**Prompt**:
> In the Nexus-Hub repo, update two skill bodies to document the template composition-strategy vocabulary adopted from GitHub Spec Kit's presets system (see docs/v3.6.0/comparison-spec-kit.md candidate N3b and Section 3 row 3):
...
**Prompt**:
> Edit `docs/policy/mcp-reverse-engineering-matrix.md` to add a row for each declined candidate from docs/v3.6.0/comparison-spec-kit.md Section 6.3 / Bucket D:
...
**Prompt**:
> Using the known-gaps-tracker convention, create or update the v3.6.0 known-gaps file (e.g., `docs/v3.6.0/known-gaps.md`, matching the format of existing `docs/v*/known-gaps.md` files) with two deferred items from docs/v3.6.0/comparison-spec-kit.md Bucket C: (1) **N4 self-upgrade CLI** - deferred, low ROI (the installer re-run already covers in-place upgrade); (2) **N2b portable YAML workflow engine** - deferred, policy-disfavored as a runtime (the harness's Dynamic Workflows cover Claude Code; revisit only if cross-agent orchestration becomes a stated goal). Note each item's source candidate ID and the condition under which it would be reconsidered.
