...
- Created [docs/v2.2.0/antigravity-cli-probe.md](../../antigravity-cli-probe.md) with seven on-disk-convention fields tagged `(documented)`, `(inferred)`, or `(open)`.
...
- Three `(open)` items remain (binary name, command format, frontmatter schema) -- tracked as WN-2 / WN-3 / WN-4 in `docs/v2.2.0/known-gaps.md`.
...
- Created [docs/v2.2.0/antigravity-cli-commands-schema.md](../../antigravity-cli-commands-schema.md) confirming the file-format diff between Gemini CLI (`~/.gemini/commands/*.toml`, TOML wrapping) and Antigravity 2.0 + CLI (`~/.agent/workflows/*.md`, verbatim Markdown).
