...
| Path | Category | Notes |
|---|---|---|
| docs/v2.2.0/RELEASE_NOTES.md | 4 (active) | New artifact authored in Phase 6 (T036). Canonical v2.2.0 user-facing narrative. |
| docs/v2.2.0/installer-smoke-post.txt | 4 (active) | New artifact authored in Phase 6 (T038). Cross-OS smoke results: Windows PASS; macOS / Linux deferred. |
| docs/v2.2.0/known-gaps.md | 4 (active) | Updated for Phase 6 close (T039); status now reads "finalized for v2.2.0 release". |
| docs/v2.2.0/docs-cleanup-report-phase6.md | 4 (active) | This file. |
...
| docs/v2.2.0/eval-baseline.md | 4 (active) | Re-verified during T038 (baseline reproduces 100% recall / 63.3% precision byte-for-byte). |
| docs/v2.2.0/docs-cleanup-report-phase2.md | 3 (stale-flag candidate) | Prior-phase audit-only report. Leave in place. |
| docs/v2.2.0/docs-cleanup-report-phase4.md | 3 (stale-flag candidate) | Same as above for Phase 4. |
| docs/v2.2.0/docs-cleanup-report-phase5.md | 3 (stale-flag candidate) | Same as above for Phase 5. |
| docs/v2.2.0/development/history/ | 4 (active) | Will receive a Phase 6 session-history entry in sub-step 8.8. |
...
`docs/v2.2.0/` is currently in the **legacy flat layout** (`docs/<vSEMVER>/`). The canonical v2.1.1+ layout is `docs/versions/v<MAJOR>/v<SEMVER>/`. v2.2.0 deliberately preserves the legacy flat layout because:
