...
- [ ] Publish the v4.4 QG-446-1 default-host interpreter follow-up: the managed Windows hook commands select PowerShell, the unmodified host PATH passes its script-execution gate, and a new unfiltered Windows full profile passed 61/61 commands. The [archived local qualification](archives/v4/v4.4/development/windows-interpreter-default-host/verification.md) preserves the receipt; protected PR and post-merge checks remain open.
