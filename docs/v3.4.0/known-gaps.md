...
**Status**: v3.4.0 is in active development on `develop`. The `model-routing` plan (`docs/v3.4.0/plans/model-routing.md`) is complete (all 4 phases); a separate `adoption-nessie-and-agency-agents` plan is also in scope for v3.4.0. The model-routing release cut (version bump + tag) is handed to `/update release` and not yet run.
**Last updated**: 2026-06-14 (model-routing Phase 4)
...
| WN-v33-1 | WN | carried forward from v3.3.0; re-confirmed model-routing Phases 1-4 | 1.5, 2.4, 3.4, 4.4 (Testing and Stabilization) | Local Windows verification is partially emulated: `make` is not on PATH, so `make validate` / `make lint` / `make test` were run by invoking the underlying validators, the scanner, and pytest directly. The Phase 4 direct chain passed green (JSON integrity 253 skills; bundles-only orphan audit PASS, 0 errors + 1 pre-existing WN-v33-2 `.pyc` warning; quality pass on the edited `model-routing` skill 0/0; strict unicode-safety clean on the two edited catalog files; no-personal-paths exit 0; version sync all six surfaces match 3.3.4; hook suite 439 passed / 7 skipped; repo-level `tests/` green). Phase 4 is pure catalog Markdown + repo docs + the HTML guide (no new `.sh`), so ShellCheck is not implicated this phase. | Confirm CI `validate` / `scan` / `shellcheck` are green on the ubuntu runner. No code change expected. | Low (direct validator/scanner equivalents passed) |
