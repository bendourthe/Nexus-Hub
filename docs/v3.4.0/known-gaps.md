...
**Status**: v3.4.0 is in active development on `develop`. The `model-routing` plan (`docs/v3.4.0/plans/model-routing.md`) is complete (all 4 phases); the `adoption-nessie-and-agency-agents` plan (`docs/v3.4.0/plans/adoption-nessie-and-agency-agents.md`) is in progress (Phase 4 of 5 complete -- Phase 1 the `context-pack-builder` skill, Phase 2 the Aider + Windsurf platform integrations, Phase 3 the `session-query` extension to Obsidian + exported ChatGPT/Gemini history, Phase 4 the optional Kimi + Qwen + OpenClaw platform integrations). The release cut (version bump + tag) is handed to `/update release` and not yet run.
**Last updated**: 2026-06-15 (adoption-nessie-and-agency-agents Phase 4)
...
| WN-v33-1 | WN | carried forward from v3.3.0; re-confirmed model-routing Phases 1-4 and adoption-nessie Phases 1-4 | 1.5, 2.4, 2.5, 3.4, 4.4 (Testing and Stabilization) | Local Windows verification is partially emulated: `make` is not on PATH, so `make validate` / `make lint` / `make test` were run by invoking the underlying validators, the scanner, and pytest directly. Re-confirmed for adoption-nessie-and-agency-agents Phase 4 (the optional Kimi + Qwen + OpenClaw platform integrations -- three new `IntegrationBase` subclasses + three base templates + a shared `_write_generated` helper on `base.py` + both installers): the direct chain passed green (`tests/integrations/` 265 passed including the new `test_kimi_qwen_openclaw.py` 12 cases and the parameterized contract suite auto-covering all 13 keys = 65 contract cases; `catalog/hooks/tests/test_installer_smoke.py` 28 passed; runner workspace dry-run install rc=0 reporting all six artifacts at expected paths). ShellCheck is not on the local PATH, so the `installer.sh` edits were verified with `bash -n` (clean) and the `installer.ps1` edits with the `[Parser]::ParseFile` AST (clean); CI ShellChecks `catalog/**/*.sh` and `installer.sh` on the ubuntu runner. Note: the catalog is unchanged (256 skills; integrations are not skills), so no `make validate` skill/registry surface was touched this phase. | Confirm CI `validate` / `scan` / `shellcheck` are green on the ubuntu runner. No code change expected. | Low (direct validator/scanner equivalents passed; both installers parse clean) |
