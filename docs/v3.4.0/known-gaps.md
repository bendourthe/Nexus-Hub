...
**Status**: v3.4.0 is in active development on `develop`. The `model-routing` plan (`docs/v3.4.0/plans/model-routing.md`) Phases 1-3 of 4 are complete; a separate `adoption-nessie-and-agency-agents` plan is also in scope for v3.4.0.
**Last updated**: 2026-06-14 (model-routing Phase 3)
...
| WN-v33-1 | WN | carried forward from v3.3.0; re-confirmed model-routing Phases 1-3 | 1.5, 2.4, 3.4 (Testing and Stabilization) | Local Windows verification is partially emulated: `make` is not on PATH, so `make validate` / `make lint` / `make test` were run by invoking the underlying validators, the scanner, and pytest directly. The Phase 3 direct chain passed green (JSON integrity; bundles-only orphan audit PASS 0/0; quality pass 0 errors + 1 pre-existing WN-v33-2 warning; unicode-safety / no-personal-paths / supply-chain-iocs / workflow-security / solution-frontmatter all exit 0; version sync all six surfaces match 3.3.4; hook suite 439 passed / 7 skipped; repo-level `tests/` 415 passed). Phase 3 is pure catalog Markdown (no new `.sh`), so ShellCheck is not implicated this phase. | Confirm CI `validate` / `scan` / `shellcheck` are green on the ubuntu runner. No code change expected. | Low (direct validator/scanner equivalents passed) |
