...
base=subprocess.check_output(['git','show','83fdfa35:guides/website/nexus-hub-guide.html']).decode('utf-8')
def strip_models(s):
...
(out/'scope.json').write_text(json.dumps({'baseline':'83fdfa35','outside_models_unchanged':True,'bytes':len(html.encode()),'sha256':hashlib.sha256(html.encode()).hexdigest(),'syntax_checks':checks},indent=2))
