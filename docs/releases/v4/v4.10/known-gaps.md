...
- **Narrowed 2026-09-13**: the markup half is now covered. Eight tests render both surfaces for real - `test/weeklyBarsRender.test.ts` decodes each percent-encoded SVG data URI out of the tooltip, and `test/dashboardWeeklyBars.test.ts` reads the dashboard HTML through the public `show()` path. Together they assert that the scoped bar appears with the account-supplied label, that it is ABSENT when no scoped limit is reported, that the all-models bar survives beside it rather than being replaced, that each SVG is well formed, that an out-of-range percent cannot overflow the track, and that a hostile label is escaped on both surfaces. Both files were mutation-checked: suppressing the scoped section fails 3 of 5 tooltip tests and 2 of 3 dashboard tests, so neither is vacuous. Suite 12 -> 20 tests.
- **What remains, and it is the original point of this gap**: none of that loads the extension. The tests prove the MARKUP is right, not that VS Code RENDERS it as intended - an `<img>` of a data-URI SVG inside a trusted `MarkdownString` is exactly the kind of thing a host can refuse to paint while every string assertion still passes.
- **Suggested next step**: install `claude-usage-monitor-0.10.0.vsix`, hover the status-bar item, and open the dashboard, confirming two weekly bars with the second labelled from the account. Still worth doing before publishing the tag, since the changelog asserts the bar appears; the residual risk is now host rendering rather than string construction.
