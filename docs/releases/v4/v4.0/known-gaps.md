...
- **Contract and audit disposition follow-up on 2026-09-25**: PR #305 merged the two living CI documents at `85d40806`, and its post-merge smoke and provenance jobs passed. In a separate bounded slice, the normative lifecycle contract moved to `docs/policy/` while the dated final, workflow, and harness audits moved to `docs/archives/v4/v4.0/development/`. Each destination matched its source hash before removal; the 55 lifecycle tests, 17-command fast gate, and rename-map link comparison passed with zero newly broken links. The committed-tree lifespan detector exited 1 with 1,386 findings repository-wide and exactly two in v4.0, both plans. Protected publication and the CI plan's 62 preserved unchecked task boxes keep WN-2 open.
