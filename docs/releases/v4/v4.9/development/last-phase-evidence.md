...
The initial publication is [PR 190](https://github.com/bendourthe/Nexus-Hub/pull/190), head `4922fd9756b57f70af9c5355e4ba92e76e313092`, integration base `b6197859fcd97dac77262d4c90b03e91110c67f0`, synthetic merge `75bd1a9d0b647eae22fee1f5a1a685fd5f74d248`. The [first hosted verify job](https://github.com/bendourthe/Nexus-Hub/actions/runs/34285132269/job/102258911412) failed with two failures, 1,571 passes and 12 skips. No merge or blind job rerun followed that failure.

### Cycle 3: deterministic test setup across filesystems

The shared correction counter is now `fix_rerun_cycles_used = 3` of 3, including the two earlier local cycles. The first hosted failure is classified TEST: the leaf-swap test changed every `fstat` inode, so POSIX directory guards correctly rejected an unintended ancestor swap before the leaf assertion. The second is TEST: unspecified ledger enumeration selected entry `0003`; replacing `0000` then copied the entry onto itself and raised `SameFileError` before ledger verification.

Before editing, a private local perturbation fixture reproduced both assumptions: reversed entry enumeration raised the same `SameFileError`, and a sentinel directory stat proved the leaf-swap mock also changed directory identity. Result: two failed in 9.10 seconds. The sentinel is a deterministic boundary probe on Windows, not a claim of native POSIX execution; the hosted stack provides the actual POSIX observation.

The only code changes are in the two tests. The swap injection now changes regular-file stats only and still requires the exact `post_open_swap` refusal. The fork setup selects the explicit `0000-` entry and changes only the four-character sequence prefix, preserving the hash suffix and the real render-rejection assertion. Runtime guards, benchmark subjects, CI configuration and security assertions are unchanged. The [independent review](qualification/v4.9-stabilization-review.md) reports zero findings and independently passes both corrected perturbation cases in 8.15 seconds.

The affected full-file rerun with both perturbations passes: 97 passed, seven explicit platform skips in 385.74 seconds, exit 0. The independent two-case run overlaps that suite and is not added to its total. Ruff passes for both changed files; all seven documentation profile commands pass; strict Unicode, Markdown structure/local links and scoped whitespace checks pass. The existing benchmark candidate independently verifies four entries with unchanged identity. The [initial main Linux test job](https://github.com/bendourthe/Nexus-Hub/actions/runs/34285132234/job/102258900579) reports the same two faults with 4,820 passes and 260 skips, without an additional failure.

The [initial Windows test job](https://github.com/bendourthe/Nexus-Hub/actions/runs/34285132234/job/102258900562) finished before the stabilization commit and exposed one additional checkout contract: 14 golden comparisons failed, with 449 passes and four skips. Its hook, installer/validator and interpreter groups passed. The retained log shows CRLF expected bytes versus LF output; the workflow uploaded no report artifact, so individual hosted tracebacks beyond that tail are unavailable. A real local `git -c core.autocrlf=true checkout-index` reproduction produced CRLF in all 14 expected JSON/SARIF files and reproduced 14 failures with three passes in 6.15 seconds. This is not inferred solely from the failure count.

The same uncommitted cycle-3 stabilization now pins `tests/fixtures/security-audit/**` to LF in `.gitattributes`, matching the existing immutable audit fixture convention. No expected bytes or assertions are relaxed. A fresh real Git checkout has zero CRLF goldens; both complete envelope/SARIF files pass against those checkout bytes: 152 passed in 22.79 seconds. That suite is disjoint from the 97-test filesystem/ledger rerun. Candidate verification still passes with four entries; no runtime subject input changed. All first-run checks are terminal: 24 passed, four failed (the two Linux jobs, Windows tests and their aggregate), one inapplicable rendering job skipped. Every bootstrap and installer smoke job passed on its supported operating system. All three reproduced causes are included in this single corrective commit.

This correction is one narrowly scoped stabilization commit, without amending the original Phase 7 commit. The user's instruction to resolve failures and finish integration authorizes this corrective publication; exact-SHA required checks and all supported installer jobs must pass before merging. The final checked SHA, job links, artifact IDs and merge SHA will be attached to the PR integration handoff.
