...
The [Cursor hook documentation](https://cursor.com/docs/hooks) and [Aider configuration reference](https://aider.chat/docs/config/aider_conf.html) support the new policy delivery. Claude attribution was verified earlier in this cycle. Other behavioral-default rows retain their previous dated evidence; no fresh verification is asserted for those keys. The live Codex roster was enumerated through its native CLI. The advisory prompting check reports DRIFTED: gpt-6-astra is recorded in the profile roster but absent from the six returned CLI ids. The four official public model catalogs still document all sixteen mapped selections, so the dated offline model-map evidence is refreshed without changing those selections. API availability is distinct from an individual host roster. No prompting-profile freshness marker changes; a later /tune-prompting run can reconcile that advisory difference.
