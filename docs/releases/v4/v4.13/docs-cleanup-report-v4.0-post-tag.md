...
PR #306 passed 32 hosted checks with one expected skip, merged to `develop` at `e3b51879`, and post-merge run 36120663990 passed smoke and provenance. The lifespan detector on merged `develop` reproduced 1,386 findings repository-wide and exactly the same two v4.0 plans. The proposed plan disposition is still a separate decision; this publication does not waive the 62-box retention rule.
