...
**A "promote nothing" disposition can be quietly contradicted later.** `TestNothingWasPromoted` asserts that none of the four candidate descriptions appears in its shipped `SKILL.md`. Each of those four checks is paired with a mutation control that plants the candidate text into a copy and confirms the check trips, so a predicate that could never match is itself caught. That is the module's existing `has_teeth` pattern applied to a file-content check.
