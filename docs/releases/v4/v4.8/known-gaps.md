...
- **Status: INVESTIGATED, NOT FIXED.** No root cause was found, so nothing was changed. Adding a retry, widening a timeout, or marking the tests flaky would each convert an unexplained failure into a silent one, which is worse than leaving it visible. This is the one v4.8.0 open item that is not closed.
- **Suggested next step, concretely runnable**: reproduce under the condition that distinguishes it (load), not by repetition. `python scripts/ci/run.py --profile full` while the machine is otherwise busy, or `pytest tests/ -q -p no:randomly --basetemp=<a path outside the OneDrive tree>` to separate the load hypothesis from the redirection one. Only then apply `[[flaky-test-detector]]`. Treat `WN-I` as covering the family (Windows tests that spawn subprocesses or manipulate filesystem state under load), and investigate both sites together, since a shared cause is likelier than two coincident ones.
