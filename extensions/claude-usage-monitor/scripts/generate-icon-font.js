/**
 * Generates a WOFF2 icon font from the Claude SVG icon.
 *
...
const SVG_FILE = path.join(ICONS_DIR, "claude.svg");
...
const svgFont = [
  ...
  "<defs>",
  `<font id="claude-icons" horiz-adv-x="${UNITS_PER_EM}">`,
  `<font-face font-family="claude-icons" units-per-em="${UNITS_PER_EM}" ascent="${UNITS_PER_EM}" descent="0" />`,
  `<missing-glyph horiz-adv-x="${UNITS_PER_EM}" />`,
  `<glyph unicode="&#x${CODEPOINT.toString(16).toUpperCase()};" glyph-name="claude" horiz-adv-x="${UNITS_PER_EM}" d="${fontPath}" />`,
  "</font>",
  "</defs>",
  "</svg>",
].join("\n");
...
fs.writeFileSync(path.join(FONTS_DIR, "claude-icons.woff2"), woff2Buffer);
fs.writeFileSync(path.join(FONTS_DIR, "claude-icons.ttf"), ttfBuffer);

console.log("Generated fonts/claude-icons.woff2 and fonts/claude-icons.ttf");
