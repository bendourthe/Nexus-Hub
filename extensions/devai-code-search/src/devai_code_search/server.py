...
from devai_code_search.config import CodeSearchConfig, index_dir_for, resolve_config
from devai_code_search.indexer import index_codebase
from devai_code_search.search_keyword import KeywordIndex
from devai_code_search.store import clear_index as store_clear_index
from devai_code_search.store import index_lock, load_index
from devai_code_search.types import IndexState, IndexStatus
...
async def run_server() -> None:
    ...
    server = Server("devai-code-search")
    ...
    logger.info("Starting devai-code-search (stdio transport)")
...
def _handle_search(arguments: dict, config: CodeSearchConfig) -> list[TextContent]:
    ...
    if mode != "keyword":
        raise NotImplementedError(
            "devai-code-search v1.0.0 supports mode='keyword' only. "
