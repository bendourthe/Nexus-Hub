"""Configuration resolution for devai-code-search.

Mirrors the pattern used by devai-skill-server: explicit DEVAI_HUB_ROOT
...
DEFAULT_INDEX_DIR_NAME = ".devai/code-index"
...
def _find_hub_root() -> Path | None:
    ...
    Matches devai-skill-server's pattern: explicit env -> module-parent-walk -> home fallback.
    ...
    if env_root:
        ...
        if candidate.exists():
            ...
            return candidate
        logger.warning("DEVAI_HUB_ROOT set to %s but path does not exist", candidate)
...
def resolve_config() -> CodeSearchConfig:
    ...
    return CodeSearchConfig(hub_root=_find_hub_root())
