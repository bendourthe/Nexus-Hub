...
from nexus_skill_server.config import ServerConfig, resolve_config


def test_resolve_with_env_var(sample_skills_json: Path, monkeypatch):
    """Hub root resolves from NEXUS_HUB_ROOT env var."""
    monkeypatch.setenv("NEXUS_HUB_ROOT", str(sample_skills_json))
...
def test_resolve_with_invalid_env_var(tmp_path: Path, monkeypatch):
    """Invalid NEXUS_HUB_ROOT falls through to other resolution methods."""
    monkeypatch.setenv("NEXUS_HUB_ROOT", str(tmp_path / "nonexistent"))
    config = resolve_config()
...
def test_resolve_embedding_provider(monkeypatch):
    ...
    monkeypatch.delenv("NEXUS_HUB_ROOT", raising=False)
    config = resolve_config()
...
def test_resolve_default_embedding_provider(monkeypatch):
    ...
    monkeypatch.delenv("NEXUS_EMBEDDING_PROVIDER", raising=False)
    monkeypatch.delenv("NEXUS_HUB_ROOT", raising=False)
    config = resolve_config()
