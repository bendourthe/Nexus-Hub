...
So the monitor reconstructs the drawdown: private-repository, GitHub-hosted, standard-runner minutes, weighted per line item. Windows and macOS minutes consume the quota faster than Linux ones - GitHub has withdrawn the page that published the multipliers, but a completed month on a real account confirmed the weighting exists (an unweighted model predicted 1,724 minutes where GitHub's own panel showed a saturated 2,000).

**The weights are derived, not tabulated** (0.4.0). Each item's weight is its own `pricePerUnit` divided by the standard Linux rate seen in the same billing payload. The widely-cited 1x / 2x / 10x multipliers were never an independent taxonomy - they are exactly the pre-2026 price ratios ($0.008 / $0.016 / $0.080), and GitHub's 2026-01-01 price cut moved the same mechanism to 1x / 1.67x / 10.33x. Deriving the ratio tracks price changes, new runner types, and ARM variants with no table to maintain.

Full write-up of when minutes are consumed at all: [when Actions minutes are consumed](../../docs/policy/github-actions-minute-consumption.md). Why the weights are derived rather than hardcoded, and what that decision beat: [the decision record](../../docs/decisions/implemented/architecture/2026-08-22-derive-actions-drawdown-weights-from-price.md). Per-month evidence and the falsifier: [the drawdown ledger](../../docs/v3/v3.18/development/github-drawdown-ledger.md).
