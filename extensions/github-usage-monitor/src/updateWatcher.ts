...
/**
 * True when a NEWER build is installed than the one this window is running.
 *
 * A window that has not been reloaded since an install keeps executing its old code
 * indefinitely. That is normally just a stale UI - but this extension writes global
 * configuration, and global writes fire `onDidChangeConfiguration` in EVERY window.
 * So a stale window is not a passive bystander: it is an active participant that can
 * drive a loop in windows running perfectly good code.
 *
 * Observed 2026-08-11: reloading one window and testing there, while another window
 * kept running a build whose reconciler rewrote the billing owner on every refresh.
 * Six fixes shipped into the reloaded window and none of them could help, because the
 * misbehaving participant was never reloaded.
 *
 * A stale window should therefore defer: keep displaying, stop writing.
 */
export function runningIsStale(context: vscode.ExtensionContext): boolean {
  const running = String((context.extension.packageJSON as { version?: unknown }).version ?? "");
  if (running === "") return false;
  const installed = vscode.extensions.getExtension(context.extension.id);
  const onDisk = installed === undefined
    ? ""
    : String((installed.packageJSON as { version?: unknown }).version ?? "");
  // Only a KNOWN difference counts. An unreadable registry is not evidence of
  // staleness, and treating it as such would silence a window that is up to date.
  return onDisk !== "" && onDisk !== running;
}
