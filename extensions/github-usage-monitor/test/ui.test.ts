...
describe("dashboard and settings", () => {
  ...
  it("creates and reuses dashboard and settings panels", () => {
    ...
    const settings = new SettingsPanel(); settings.show(); settings.show(); expect(webviewPanels).toHaveLength(2); expect(webviewPanels[1]?.webview.html).toContain("GitHub Usage Settings");
  });
});
