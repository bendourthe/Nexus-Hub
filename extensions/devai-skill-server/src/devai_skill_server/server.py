...
from devai_skill_server.config import resolve_config
...
NOT_LOADED_MSG = (
    "DevAI-Hub skill catalog not found. "
    "Set the DEVAI_HUB_ROOT environment variable to your DevAI-Hub repository root, "
    "or run the DevAI-Hub installer (install.sh / install.bat)."
)
...
async def run_server() -> None:
    ...
    server = Server("devai-skill-server")
    ...
    async def list_tools() -> list[Tool]:
        return [
            Tool(
                name="search_skills",
                description=(
                    "Search DevAI-Hub skills by keyword or natural language query. "
            ...
            Tool(
                name="get_skill",
                description=(
                    "Retrieve a specific DevAI-Hub skill by exact name (kebab-case). "
            ...
            Tool(
                ...
                description="List all DevAI-Hub skill categories with skill counts and names.",
    ...
    logger.info("Starting devai-skill-server (stdio transport)")
